...
class WebinarController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        // $stepCount = empty(getGeneralOptionsSettings('direct_publication_of_courses')) ? 5 : 6;
        $stepCount = 4;
    ...
    public function store(Request $request)
    {
        ...
        if ($request->title) {
            $request->merge(['slug' => Webinar::makeSlug($request->title)]);
    ...
    public function edit(Request $request, $id, $step = 1)
    {
        ...
        $stepCount = 4;
        $data = [
            'pageTitle' => trans('webinars.new_page_title_step'),
            'currentStep' => $step,
            'isOrganization' => $isOrganization,
            'userLanguages' => getUserLanguagesLists(),
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!$directPublicationOfCourses and ((!$isDraft) or (!$isDraft))) {
        ...
        if ($directPublicationOfCourses and !$isDraft) {
        ...
        if ($currentStep == 1) {
            ...
            $userCoursesCapacityLimited = $userPackage->checkPackageLimit('courses_capacity');
        ...
        if ($getNextStep) {
            ...
            if ($currentStep == 4) {
                ...
                // $url = '/panel/webinars/' . $webinar->id . '/step/' . min($nextStep, $stepCount);
                $url = '/panel/webinars/' . $webinar->id . '/step/4';
            }
        ...
        if ($status != Webinar::$active and !$isDraft and !$webinarRulesRequired) {
