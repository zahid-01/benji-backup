...
class WebinarController extends Controller
{
    ...
    public function course($slug, $justReturnData = false)
    {
        ...
        if (request()->file_id) {
            $requestVides = File::whereIn('file_type', File::$videoTypes)
                ->where('accessibility', 'free')->where('status', 'active')->where('id', request()->file_id)->first();
            if ($requestVides) {
                $course->video_demo_source = $requestVides->storage;
                $course->video_demo = $requestVides->file;
            }
        }

        $otherCourses = File::whereIn('file_type', File::$videoTypes)
        ...
        return view('web.jiujitsu.course.index', $data);
    }
            ...
            if (!empty($file)) {
                ...
                if ($canAccess) {
                    ...
                    if (\Illuminate\Support\Facades\File::exists(public_path($filePath))) {
                        ...
                        return view('web.jiujitsu.course.learningPage.interactive_file', $data);
                    }
