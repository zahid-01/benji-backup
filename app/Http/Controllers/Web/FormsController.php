    ...
    public function index(Request $request, $url)
    {
        ...
        if (!empty($form)) {
            ...
            if (!empty($form->start_date) and $form->start_date > time()) {
                return view('web.jiujitsu.forms.not_start', $data);
            }

            if (!empty($form->end_date) and $form->end_date < time()) {
                return view('web.jiujitsu.forms.expired', $data);
            }

            if ($form->enable_login and empty($user)) { // if enable login and user not login
                return view('web.jiujitsu.forms.please_login', $data);
            }


            $checkAccess = $this->checkAccess($form, $user);
            if (!$checkAccess) {
                return view('web.jiujitsu.forms.access_denied', $data);
            }
            ...
            if ($showWelcome) {
                return view('web.jiujitsu.forms.welcome', $data);
            }

            if ($showTanks) {
                return view('web.jiujitsu.forms.tanks', $data);
            }

            if ($hasSubmission) {
                return view('web.jiujitsu.forms.already_submitted', $data);
            }

            return view('web.jiujitsu.forms.fields', $data);
        }
