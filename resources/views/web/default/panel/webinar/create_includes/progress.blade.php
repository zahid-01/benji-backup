...
<div class="webinar-progress d-grid d-md-flex align-items-center p-15 panel-shadow bg-white rounded-sm">

    @foreach ($progressSteps as $key => $step)     
        <div class="progress-item d-flex align-items-center">
            <button type="button" data-step="{{ $key }}"
                class="js-get-next-step p-0 border-0 progress-icon d-flex align-items-center justify-content-center rounded-circle {{ $key == $currentStep ? 'active' : '' }}"
            ...
            <div class="ml-10 {{ $key == $currentStep ? '' : '' }}">
                <span
                    class="font-14 text-gray">{{ trans('webinars.progress_step', ['step' => $key, 'count' => $stepCount]) }}</span>
