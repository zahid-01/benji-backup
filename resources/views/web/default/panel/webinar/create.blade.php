...
@section('content')
    <div class="">
        ...
        <div
            ...
            {{-- <a href="/panel/webinars/{{ $webinar->id }}/step/1"
                        class="btn btn-sm btn-primary {{ $currentStep < 2 ? 'disabled' : '' }}">{{ trans('webinars.previous') }}</a>
                ...
                <button type="button" id="getNextStep" class="btn btn-sm btn-primary ml-15"
                    @if ($currentStep >= $stepCount) disabled @endif>{{ trans('webinars.next') }}</button>
            </div> --}}
            ...
            <div class="d-flex align-items-center">
                ...
                @if ($currentStep < $stepCount)
                    <button type="button" id="getNextStep"
                        class="btn btn-sm btn-primary ml-15">{{ trans('webinars.next') }}</button>
                @endif
            </div>
                ...
                <div class="mt-20 mt-md-0">
                    ...
                    @if (!empty($webinar) and $webinar->creator_id == $authUser->id)
                        <a href="/panel/webinars/{{ $webinar->id }}/delete?redirect_to=/panel/webinars"
                            class="delete-action webinar-actions btn btn-sm btn-danger mt-20 mt-md-0">{{ trans('public.delete') }}</a>
                    @endif
                </div>
            @endif
        </div>
...
@push('scripts_bottom')
    <script>
        ...
        var hasZoomApiToken =
            '{{ (!empty($authUser->zoomApi) and !empty($authUser->zoomApi->api_key) and !empty($authUser->zoomApi->api_secret)) ? 'true' : 'false' }}';
