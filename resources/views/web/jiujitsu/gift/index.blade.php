...
@section('content')
    <div class="container mt-50">
        <div class="text-center">
            <h1 class="font-36 font-bold text-dark">{{ $pageTitle }}</h1>
            <p class="font-16 text-slate-500 mt-10">{{ $titleHint }}</p>
        </div>

        <div class="mt-50 rounded-lg border border-gray300">
            <div class="row">
                <div class="col-12 col-md-6 px-30 px-lg-80 py-30 py-lg-50 border-right">
                    <h3 class="font-24 font-bold mb-25">{{ trans('update.recipient_information') }}</h3>
                    ...
                    <form action="/gift/{{ $itemType }}/{{ $item->slug }}" method="post">
                        ...
                        <button type="submit" class="btn btn-primary btn-block mt-4">{{ trans('update.proceed_to_checkout') }}</button>
                    </form>
                </div>

                <div class="col-12 col-md-6 flex-center px-30 px-lg-80 py-30 py-lg-50">
                    <div class="gift-item-card flex">

                        @if($itemType == 'course')
                            @include('web.jiujitsu.gift.course_card',['webinar' => $item])
                        @elseif($itemType == 'bundle')
                            @include('web.jiujitsu.gift.bundle_card',['bundle' => $item])
                        @elseif($itemType == 'product')
                            @include('web.jiujitsu.gift.product_card',['product' => $item])
                        @endif
