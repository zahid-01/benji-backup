            ...
            <div class="flex items-center">
                ...
                <div class="">
                    @if(!empty($floatingBar->title))
                        <h5 class="font-16 font-bold" style="{{ !empty($floatingBar->title_color) ? "color: $floatingBar->title_color" : '' }}">{{ $floatingBar->title }}</h5>
                    @endif

                    @if(!empty($floatingBar->description))
                        <div class="text-sm" style="{{ !empty($floatingBar->description_color) ? "color: $floatingBar->description_color" : '' }}">{{ $floatingBar->description }}</div>
                    @endif
                </div>
            </div>
