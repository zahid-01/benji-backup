...
@if(!empty($cookieSecuritySettings['cookie_settings_modal_message']) and !empty(strip_tags($cookieSecuritySettings['cookie_settings_modal_message'])))
    <div class="cookie-security-dialog p-20 bg-gray rounded-lg">
        <h3 class="text-sm font-bold text-white">{{ trans('update.your_privacy') }}</h3>
        <p class="mt-5 text-white text-sm">{{ trans('update.your_privacy_hint') }}</p>

        <div class="mt-10 flex flex-wrap items-center">
            <button type="button" class="js-accept-all-cookies btn btn-primary btn-sm grow mr-0 mr-md-5">{{ trans('update.accept_all_cookies') }}</button>
            <button type="button" class="js-cookie-customize-settings btn btn-light btn-sm grow mt-10 mt-md-0">{{ trans('update.customize_settings') }}</button>
        </div>
    </div>

    <div id="cookieSecurityModal" class="hidden">
                ...
                @foreach($cookieSecuritySettings['cookie_settings_modal_items'] as $cookieModalItemKey => $cookieModalItem)
                    ...
                    <div class="cookie-settings-modal-items-card mb-15">
                        <div class="form-group flex items-center mb-0">
                            ...
                            <label class="cursor-pointer text-sm text-slate-500 mb-0 {{ $isRequiredModalItem ? 'c-not-allowed' : '' }}" for="cookieModalItem{{ $cookieModalItemKey }}_record">{{ $cookieModalItem['title'] }}</label>
                            ...
                            @if(!empty($cookieModalItem['description']))
                                <button type="button" class="js-cookie-settings-modal-items-help btn-ghost ml-15">
                                    <i data-feather="help-circle" width="20" height="20" class="text-slate-500"></i>
                                </button>
                            @endif
                        </div>

                        <ul class="cookie-settings-modal-items-card__description">
                            <li class="text-sm text-slate-500">{!! $cookieModalItem['description'] !!}</li>
                        </ul>
                    </div>
        ...
        <div class="flex flex-wrap items-center mt-4 pt-15 border-top">
            <button type="button" class="js-store-customize-cookies d-inline-flex d-mhidden btn btn-primary btn-sm">{{ trans('update.confirm') }}</button>
            <button type="button" class="js-store-customize-cookies hidden d-md-inline-flex btn btn-primary btn-sm">{{ trans('update.confirm_my_choices') }}</button>

            <button type="button" class="js-accept-all-cookies d-inline-flex d-mhidden btn btn-outline-primary btn-sm mx-15">{{ trans('update.accept_all') }}</button>
            <button type="button" class="js-accept-all-cookies hidden d-md-inline-flex btn btn-outline-primary btn-sm mx-15">{{ trans('update.accept_all_cookies') }}</button>

            <button type="button" class="btn-ghost close-swl ml-auto text-sm text-danger">{{ trans('public.cancel') }}</button>
        </div>
    </div>
