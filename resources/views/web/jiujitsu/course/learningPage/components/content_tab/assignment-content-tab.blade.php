...
>
        ...
        <span class="chapter-icon bg-gray300 mr-10">
            <i data-feather="feather" class="text-slate-500" width="16" height="16"></i>
        </span>
    ...
    <div>
        <div class="">
            ...
            @if(empty($itemHistory) or ($itemHistory->status == \App\Models\WebinarAssignmentHistory::$notSubmitted))
                <span class="text-danger text-sm block">{{ trans('update.assignment_history_status_not_submitted') }}</span>
            @else
                @switch($itemHistory->status)
                    @case(\App\Models\WebinarAssignmentHistory::$passed)
                        <span class="text-primary text-sm block">{{ trans('quiz.passed') }}</span>
                        @break
                    @case(\App\Models\WebinarAssignmentHistory::$pending)
                        <span class="text-warning text-sm block">{{ trans('public.pending') }}</span>
                        @break
                    @case(\App\Models\WebinarAssignmentHistory::$notPassed)
                        <span class="text-sm block text-danger">{{ trans('quiz.failed') }}</span>
                        @break
                @endswitch
            @endif
        </div>
        ...
        <div class="tab-item-info mt-15">
            ...
            <div class="block mt-10 text-sm text-slate-500">
