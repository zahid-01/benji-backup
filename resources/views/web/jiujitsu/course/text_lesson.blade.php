        ...
        <div class="container h-100">
            <div class="row h-100 items-center justify-center text-center">
                <div class="col-12 col-md-9 col-lg-7">
                    ...
                    <h1 class="text-3xl text-white font-bold">{{ $textLesson->title }}</h1>

                    <div class="mt-4 font-16 font-medium text-white">
                    ...
                    <div class="mt-4 font-16 font-medium text-white">
                        <span>{{ trans('product.course') }}: <a href="{{ $course->getUrl() }}" class="font-16 font-medium text-white text-decoration-underline">{{ $course->title }}</a></span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <section class="container mt-10 mt-md-40">
        <div class="row">
            <div class="col-12 col-lg-8">
                ...
                <div class="mt-6 row items-center">
                    <div class="col-12 col-md-5">
                        @if(auth()->check())
                            <div class="flex items-center justify-between">
                                <label class="cursor-pointer font-medium" for="readLessonSwitch">{{ trans('public.i_passed_this_lesson') }}</label>
            ...
            <div class="col-12 col-lg-4">
                ...
                <div class="rounded-lg shadow-lg mt-8 p-20 course-teacher-card flex items-center flex-col">
                    ...
                    <h3 class="mt-10 font-20 font-bold text-black">{{ $course->teacher->full_name }}</h3>
                    <span class="mt-5 font-medium text-slate-500">{{ trans('product.product_designer') }}</span>

                    @include('web.jiujitsu.includes.webinar.rate',['rate' => $course->teacher->rates()])

                    <div class="user-reward-badges flex flex-wrap items-center mt-4">
                    ...
                    <div class="mt-25 flex flex-row items-center justify-center w-full">
                ...
                @if(!empty($textLesson->attachments) and count($textLesson->attachments))
                    <div class="shadow-lg rounded-lg bg-white px-15 px-md-25 py-20 mt-6">
                        <h3 class="category-filter-title font-16 font-bold text-dark-blue">{{ trans('public.attachments') }}</h3>

                        <ul class="p-0 m-0 pt-10">
                            @foreach($textLesson->attachments as $attachment)
                                <li class="mt-10 p-10 rounded bg-info-light text-sm font-medium text-dark-blue flex items-center justify-between text-ellipsis">
                                    <span class="">{{ $attachment->file->title }}</span>

                                    <a href="{{ $course->getLearningPageUrl() }}?type=file&item={{ $attachment->file->id }}" target="_blank">
                                        <i data-feather="download-cloud" width="20" class="text-black"></i>
                                    </a>
                                </li>
                            @endforeach
                        </ul>
                ...
                @if(!empty($course->textLessons) and count($course->textLessons))
                    <div class="shadow-lg rounded-lg bg-white px-15 px-md-25 py-20 mt-6">
                        <h3 class="category-filter-title font-16 font-bold text-dark-blue">{{ trans('public.course_sessions') }}</h3>

                        <div class="p-0 m-0 pt-10">
                            @foreach($course->textLessons as $lesson)
                                <a href="{{ $course->getUrl() }}/lessons/{{ $lesson->id }}/read"
                                   class="block mt-10 px-10 py-15 rounded text-sm font-medium text-ellipsis @if($lesson->id == $textLesson->id) bg-primary text-white @else bg-info-light text-dark-blue @endif">
