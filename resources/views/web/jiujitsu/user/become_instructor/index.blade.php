        ...
        <div class="row login-container">
            ...
            <div class="col-12 col-md-6">
                <div class="login-card">
                    <h1 class="font-20 font-bold">{{ trans('site.become_instructor') }}</h1>

                    <form method="Post" action="/become-instructor" class="mt-8">
                        {{ csrf_field() }}

                        <div class="form-group">
                            <label class="js-instructor-label font-medium text-dark-blue {{ !$isInstructorRole ? 'hidden' : '' }}">{{ trans('update.instructor_occupations') }}</label>
                            <label class="js-organization-label font-medium text-dark-blue {{ !$isOrganizationRole ? 'hidden' : '' }}">{{ trans('update.organization_occupations') }}</label>
                            ...
                            <div class="flex flex-wrap mt-5">
                                ...
                                @foreach($categories as $category)
                                    @if(!empty($category->subCategories) and count($category->subCategories))
                                        @foreach($category->subCategories as $subCategory)
                                            <div class="checkbox-button mr-15 mt-10 text-sm">
                                ...
                                @if($errors->has('occupations'))
                                    <div class="text-danger text-sm">{{ $errors->first('occupations') }}</div>
                                @endif
                            </div>
                        </div>

                        <div class="form-group">
                            <label class="js-instructor-label font-medium text-dark-blue {{ !$isInstructorRole ? 'hidden' : '' }}">{{ trans('update.instructor_select_role') }}</label>
                            <label class="js-organization-label font-medium text-dark-blue {{ !$isOrganizationRole ? 'hidden' : '' }}">{{ trans('update.organization_select_role') }}</label>
                        ...
                        <div class="form-group">
                            <label class="js-instructor-label font-medium text-dark-blue {{ !$isInstructorRole ? 'hidden' : '' }}">{{ trans('update.instructor_certificate_and_documents') }}</label>
                            <label class="js-organization-label font-medium text-dark-blue {{ !$isOrganizationRole ? 'hidden' : '' }}">{{ trans('update.organization_certificate_and_documents') }}</label>
                        ...
                        <div class="form-group">
                            <label class="js-instructor-label font-medium text-dark-blue {{ !$isInstructorRole ? 'hidden' : '' }}">{{ trans('update.instructor_select_account_type') }}</label>
                            <label class="js-organization-label font-medium text-dark-blue {{ !$isOrganizationRole ? 'hidden' : '' }}">{{ trans('update.organization_select_account_type') }}</label>
                        ...
                        <div class="js-bank-specifications-card">
                            @if(!empty($user) and !empty($user->selectedBank) and !empty($user->selectedBank->bank))
                                @foreach($user->selectedBank->bank->specifications as $specification)
                                    ...
                                    @endphp
                                    <div class="form-group">
                                        <label class="font-medium text-dark-blue">{{ $specification->name }}</label>
                        ...
                        <div class="form-group">
                            <label class="js-instructor-label font-medium text-dark-blue {{ !$isInstructorRole ? 'hidden' : '' }}">{{ trans('update.instructor_identity_scan') }}</label>
                            <label class="js-organization-label font-medium text-dark-blue {{ !$isOrganizationRole ? 'hidden' : '' }}">{{ trans('update.organization_identity_scan') }}</label>
                            <div class="input-group">
                                <div class="input-group-prepend">
                        ...
                        <div class="form-group">
                            <label class="js-instructor-label font-medium text-dark-blue {{ !$isInstructorRole ? 'hidden' : '' }}">{{ trans('update.instructor_extra_information') }}</label>
                            <label class="js-organization-label font-medium text-dark-blue {{ !$isOrganizationRole ? 'hidden' : '' }}">{{ trans('update.organization_extra_information') }}</label>
                        ...
                        <button type="submit" class="btn btn-primary btn-block mt-4">{{ (!empty(getRegistrationPackagesGeneralSettings('show_packages_during_registration')) and getRegistrationPackagesGeneralSettings('show_packages_during_registration')) ? trans('webinars.next') : trans('site.send_request') }}</button>
                    </form>
