    ...
    @if(!empty($user))
        <div class="text-center">
            ...
            <p class="text-sm font-medium text-slate-500 mt-6">
                <span class="block">{{ trans('update.this_course_requires_n_points',['points' => $bundle->points]) }}</span>
        ...
        <div class="flex items-center mt-25">
            <a href="{{ ($user->getRewardPoints() >= $bundle->points) ? '/bundles/'. $bundle->slug .'/points/apply' : '#' }}" class="btn btn-sm grow {{ ($user->getRewardPoints() >= $bundle->points) ? 'btn-primary js-buy-course-with-point' : 'bg-gray300 text-slate-500 disabled' }}">{{ trans('update.buy') }}</a>
