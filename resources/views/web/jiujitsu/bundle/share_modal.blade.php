<div class="hidden" id="courseShareModal">
    ...
    <div class="text-center">
        ...
        <p class="mt-4 text-sm">{{ trans('public.share_this_course_with_others') }}</p>

        <div class="relative flex items-center justify-between p-15 mt-15 border border-gray250 rounded-sm mt-5">
            <div class="js-course-share-link font-bold px-16 text-ellipsis text-sm">{{ $bundle->getUrl() }}</div>

            <button type="button" class="js-course-share-link-copy btn btn-primary btn-sm text-sm font-medium flex-none" data-toggle="tooltip" data-placement="top" title="{{ trans('public.copy') }}">{{ trans('public.copy') }}</button>
        </div>
