...
@section('content')
            ...
            <div class="row">
                <div class="col-6 col-md-3 flex items-center justify-center">
                    <div class="flex flex-col items-center text-center">
                        ...
                        <strong class="text-3xl font-bold text-dark-blue mt-5">{{ $pendingReserveCount }}</strong>
                        <span class="font-16 text-slate-500 font-medium">{{ trans('panel.pending_appointments') }}</span>
                    </div>
                </div>
                ...
                <div class="col-6 col-md-3 flex items-center justify-center">
                    <div class="flex flex-col items-center text-center">
                        ...
                        <strong class="text-3xl font-bold text-dark-blue mt-5">{{ $totalReserveCount }}</strong>
                        <span class="font-16 text-slate-500 font-medium">{{ trans('panel.total_meetings') }}</span>
                    </div>
                </div>

                <div class="col-6 col-md-3 flex items-center justify-center mt-5 mt-md-0">
                    <div class="flex flex-col items-center text-center">
                        <img src="/assets/default/img/activity/38.svg" width="64" height="64" alt="">
                        <strong class="text-3xl font-bold text-dark-blue mt-5">{{ handlePrice($sumReservePaid) }}</strong>
                ...
                <div class="col-6 col-md-3 flex items-center justify-center mt-5 mt-md-0">
                    <div class="flex flex-col items-center text-center">
                        ...
                        <strong class="text-3xl font-bold text-dark-blue mt-5">{{ convertMinutesToHourAndMinute($activeHoursCount / 60) }}</strong>
                        <span class="font-16 text-slate-500 font-medium">{{ trans('panel.active_hours') }}</span>
                    </div>
                </div>
    ...
    <section class="mt-8 pb-50 mb-50">
        <form action="/panel/meetings/requests?{{ http_build_query(request()->all()) }}" method="get" class="flex align-items-start align-items-md-center justify-between flex-col flex-md-row">
            ...
            <div class="flex items-center flex-row-reverse flex-md-row justify-content-start justify-content-md-center mt-4 mt-md-0">
                <label class="cursor-pointer mb-0 mr-10 text-slate-500 text-sm font-medium" for="openMeetingResult">{{ trans('panel.show_only_open_meetings') }}</label>
                                    ...
                                    <tr>
                                        <td class="text-left">
                                            <div class="user-inline-avatar flex items-center">
                                                ...
                                                <div class=" ml-2">
                                                    <span class="block font-medium">{{ $ReserveMeeting->user->full_name }}</span>
                                                    <span class="mt-5 text-sm text-slate-500 block">{{ $ReserveMeeting->user->email }}</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="align-middle">
                                            <span class="font-medium">{{ trans('update.'.$ReserveMeeting->meeting_type) }}</span>
                                        </td>
                                        <td class="align-middle">
                                            <span class="font-medium">{{ dateTimeFormat($ReserveMeeting->start_at, 'D') }}</span>
                                        </td>
                                        <td class="align-middle">
                                            <span>{{ dateTimeFormat($ReserveMeeting->start_at, 'j M Y') }}</span>
                                        </td>
                                        <td class="align-middle">
                                            <div class="d-inline-flex items-center rounded bg-gray200 py-5 px-15 text-sm font-medium">
                                        ...
                                        <td class="font-medium align-middle">{{ handlePrice($ReserveMeeting->paid_amount) }}</td>
                                        ...
                                        <td class="align-middle">
                                            @switch($ReserveMeeting->status)
                                                @case(\App\Models\ReserveMeeting::$pending)
                                                    <span class="font-medium">{{ trans('public.pending') }}</span>
                                                    @break
                                                @case(\App\Models\ReserveMeeting::$open)
                                                    <span class="text-primary font-medium">{{ trans('public.open') }}</span>
                                                    @break
                                                @case(\App\Models\ReserveMeeting::$finished)
                                                    <span class="font-medium">{{ trans('public.finished') }}</span>
                                                    @break
                                                @case(\App\Models\ReserveMeeting::$canceled)
                                                    <span class="font-medium">{{ trans('public.canceled') }}</span>
                                                    @break
                                            @endswitch
                                        </td>
                                                    ...
                                                    <div class="dropdown-menu menu-lg">
                                                        ...
                                                        @if(getFeaturesSettings('agora_for_meeting') and $ReserveMeeting->meeting_type != 'in_person')
                                                            @if(empty($ReserveMeeting->session))
                                                                <button type="button" data-item-id="{{ $ReserveMeeting->id }}" data-date="{{ dateTimeFormat($ReserveMeeting->start_at, 'j M Y H:i') }}"
                                                                        class="js-add-meeting-session btn-ghost webinar-actions block mt-10 text-primary">{{ trans('update.create_a_session') }}</button>
                                                            @elseif($ReserveMeeting->status == \App\Models\ReserveMeeting::$open)
                                                                <button type="button" data-item-id="{{ $ReserveMeeting->id }}" data-date="{{ dateTimeFormat($ReserveMeeting->start_at, 'j M Y H:i') }}" data-link="{{ $ReserveMeeting->session->getJoinLink() }}"
                                                                        class="js-join-meeting-session btn-ghost webinar-actions block mt-10 text-primary">{{ trans('update.join_to_session') }}</button>
                                                            @endif
                                                        @endif
                                                        ...
                                                        @if($ReserveMeeting->meeting_type != 'in_person' and !empty($ReserveMeeting->link) and $ReserveMeeting->status == \App\Models\ReserveMeeting::$open)
                                                            <button type="button" data-reserve-id="{{ $ReserveMeeting->id }}"
                                                                    class="js-join-reserve btn-ghost webinar-actions block mt-10">{{ trans('footer.join') }}</button>
                                                        @endif

                                                        @if($ReserveMeeting->meeting_type != 'in_person')
                                                            <button type="button" data-item-id="{{ $ReserveMeeting->id }}"
                                                                    class="add-meeting-url btn-ghost webinar-actions block mt-10">{{ trans('panel.create_link') }}</button>
                                                        @endif

                                                        <a href="{{ $ReserveMeeting->addToCalendarLink() }}" target="_blank" class="webinar-actions block mt-10 font-weight-normal">{{ trans('public.add_to_calendar') }}</a>

                                                        <button type="button"
                                                                data-user-id="{{ $ReserveMeeting->user_id }}"
                                                                data-item-id="{{ $ReserveMeeting->id }}"
                                                                data-user-type="student"
                                                                class="contact-info btn-ghost webinar-actions block mt-10">{{ trans('panel.contact_student') }}</button>

                                                        <button type="button" data-id="{{ $ReserveMeeting->id }}" class="webinar-actions js-finish-meeting-reserve block btn-ghost mt-10 font-weight-normal">{{ trans('panel.finish_meeting') }}</button>
                                                    </div>
                                                </div>
                                            @endif
                                        </td>
    ...
    <div class="hidden" id="liveMeetingLinkModal">
        ...
        <form action="/panel/meetings/create-link" method="post">
            ...
            <p class="font-medium text-sm text-slate-500">{{ trans('panel.add_live_meeting_link_hint') }}</p>
    ...
    @include('web.jiujitsu.panel.meeting.join_modal')
    @include('web.jiujitsu.panel.meeting.meeting_create_session_modal')
@endsection
