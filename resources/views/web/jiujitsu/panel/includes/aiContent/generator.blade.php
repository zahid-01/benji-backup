...
<div class="ai-content-generator-drawer bg-white">
    <div class="ai-content-generator-drawer-header flex items-center justify-between p-15">
        <div class="">
            <h5 class="font-16 font-bold">{{ trans('update.ai_content_generator') }}</h5>
            <p class="mt-5 text-sm text-slate-500">{{ trans('update.generate_content_using_AI_fast_and_premium') }}</p>
        </div>

        <button type="button" class="js-right-drawer-close flex btn-ghost">
    ...
    <div class="drawer-content p-15" data-simplebar @if((!empty($isRtl) and $isRtl)) data-simplebar-direction="rtl" @endif>
        ...
        <form action="/panel/ai-contents/generate" method="post">
            ...
            <div class="flex items-center p-15 rounded-sm bg-info-light border-gray300 mb-15">
                <div class="flex-center size-40 rounded-full bg-white">
                    <img src="/assets/default/img/ai/ai-chip.svg" alt="ai" class="" width="20px" height="20px">
                </div>
                <div class="ml-2">
                    <h5 class="block font-bold text-slate-500 text-sm">{{ trans('update.generate_content_easily') }}</h5>
                    <p class="text-sm text-slate-500">{{ trans('update.select_the_content_type_you_want_and_describe_your_requirements_and_get_the_content') }}</p>
                </div>
            </div>
            ...
            {{-- Text Fields --}}
            <div class="js-text-templates-field mt-4 mt-4 {{ (!empty($aiSettings['activate_text_service_type']) and empty($aiSettings['activate_image_service_type'])) ? '' : 'hidden' }}">
                ...
                @if(!empty(getUserLanguagesLists()))
                    <div class="js-for-service-fields form-group hidden">
                ...
                <div class="js-for-service-fields form-group hidden">
                    ...
                    <div class="invalid-feedback"></div>
                    <p class="mt-5 text-sm text-slate-500">{{ trans('update.describe_in_some_words_about_what_you_want') }}</p>
                </div>

                <div class="form-group js-question-field hidden">
                    <label class="input-label">{{ trans('update.question') }}</label>
                    <input type="text" name="question" class="js-ajax-question form-control"/>
                    <div class="invalid-feedback"></div>
                    <p class="mt-5 text-sm text-slate-500">{{ trans('update.ask_ai_what_you_want') }}</p>
                </div>


                <div class="js-service-length-field form-group hidden">
            ...
            {{-- Image Fields --}}
            <div class="js-image-templates-field {{ (!empty($aiSettings['activate_image_service_type']) and empty($aiSettings['activate_text_service_type'])) ? '' : 'hidden' }}">
                ...
                <div class="form-group js-image-question-field hidden">
                    <label class="input-label">{{ trans('update.question') }}</label>
                ...
                <div class="js-image-keyword-field form-group hidden">
                    <label class="input-label">{{ trans('update.keyword') }}</label>
                    <input type="text" name="image_keyword" class="js-ajax-image_keyword form-control"/>
                    <div class="invalid-feedback"></div>
                    <p class="mt-1 text-sm text-slate-500">{{ trans('update.describe_in_some_words_about_what_you_want') }}</p>
                </div>

                <div class="js-image-size-field form-group hidden">
            ...
            <button type="button" class="js-submit-ai-content-form btn btn-primary btn-block mt-4">{{ trans('update.generate') }}</button>
        </form>

        {{-- Text Generated --}}
        <div id="generatedTextContents" class="hidden"></div>


        {{-- Text Generated --}}
        <div class="js-image-generated mt-4 p-15 bg-info-light border-gray300 rounded-sm hidden">
            <div class="">
                <h4 class="text-sm text-slate-500">{{ trans('update.generated_content') }}</h4>
                <p class="mt-5 text-slate-500 text-sm">{{ trans('update.click_on_images_to_download_them') }}</p>
            </div>

            <div class="js-content mt-15 flex-center flex-wrap">
