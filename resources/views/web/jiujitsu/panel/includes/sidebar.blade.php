...
<div class="xs-panel-nav flex lg:hidden justify-between py-5 px-15">
    <div class="user-info flex items-center justify-between">
        ...
        <div class="user-name ml-15">
            <h3 class="font-16 font-bold">{{ $authUser->full_name }}</h3>
        </div>
    </div>

    <button class="sidebar-toggler btn-ghost flex flex-col-reverse justify-center items-center p-5 rounded-sm sidebarNavToggle" type="button">
        <span>{{ trans('navbar.menu') }}</span>
        <i data-feather="menu" width="16" height="16"></i>
    </button>
</div>

<div class="panel-sidebar pt-50 pb-25 px-4" id="panelSidebar">
    <button class="btn-ghost panel-sidebar-close sidebarNavToggle">
        <i data-feather="x" width="24" height="24"></i>
    </button>

    <div class="user-info flex items-center flex-row flex-lg-column justify-content-lg-center">
        ...
        <div class="flex flex-col items-center justify-center">
            <a href="/panel" class="user-name mt-15">
                <h3 class="font-16 font-bold text-center">{{ $authUser->full_name }}</h3>
            </a>
    ...
    <div class="flex sidebar-user-stats pb-10 ml-20 pb-lg-20 mt-15 mt-lg-30">
        <div class="sidebar-user-stat-item flex flex-col">
            <strong class="text-center">{{ $authUser->webinars()->count() }}</strong>
            <span class="text-sm">{{ trans('panel.classes') }}</span>
        </div>
        ...
        @if($authUser->isUser())
            <div class="sidebar-user-stat-item flex flex-col">
                ...
                <span class="text-sm">{{ trans('panel.following') }}</span>
            </div>
        @else
            <div class="sidebar-user-stat-item flex flex-col">
                <strong class="text-center">{{ $authUser->followers()->count() }}</strong>
                <span class="text-sm">{{ trans('panel.followers') }}</span>
            </div>
        @endif
    </div>
    ...
    <ul id="panel-sidebar-scroll" class="sidebar-menu pt-10 @if(!empty($authUser->userGroup)) has-user-group @endif @if(empty($getPanelSidebarSettings) or empty($getPanelSidebarSettings['background'])) without-bottom-image @endif" @if((!empty($isRtl) and $isRtl)) data-simplebar-direction="rtl" @endif>
        ...
        <li class="sidenav-item {{ (request()->is('panel')) ? 'sidenav-item-active' : '' }}">
            <a href="/panel" class="flex items-center">
                <span class="sidenav-item-icon mr-10">
                    @include('web.jiujitsu.panel.includes.sidebar_icons.dashboard')
                </span>
                <span class="text-sm text-dark-blue font-medium">{{ trans('panel.dashboard') }}</span>
            </a>
        </li>

        @if($authUser->isOrganization())
            <li class="sidenav-item {{ (request()->is('panel/instructors') or request()->is('panel/manage/instructors*')) ? 'sidenav-item-active' : '' }}">
                <a class="flex items-center" data-toggle="collapse" href="#instructorsCollapse" role="button" aria-expanded="false" aria-controls="instructorsCollapse">
                <span class="sidenav-item-icon mr-10">
                    @include('web.jiujitsu.panel.includes.sidebar_icons.teachers')
                </span>
                    <span class="text-sm text-dark-blue font-medium">{{ trans('public.instructors') }}</span>
                </a>
            ...
            <li class="sidenav-item {{ (request()->is('panel/students') or request()->is('panel/manage/students*')) ? 'sidenav-item-active' : '' }}">
                <a class="flex items-center" data-toggle="collapse" href="#studentsCollapse" role="button" aria-expanded="false" aria-controls="studentsCollapse">
                <span class="sidenav-item-icon mr-10">
                    @include('web.jiujitsu.panel.includes.sidebar_icons.students')
                </span>
                    <span class="text-sm text-dark-blue font-medium">{{ trans('quiz.students') }}</span>
                </a>
        ...
        <li class="sidenav-item {{ (request()->is('panel/webinars') or request()->is('panel/webinars/*')) ? 'sidenav-item-active' : '' }}">
            <a class="flex items-center" data-toggle="collapse" href="#webinarCollapse" role="button" aria-expanded="false" aria-controls="webinarCollapse">
                <span class="sidenav-item-icon mr-10">
                    @include('web.jiujitsu.panel.includes.sidebar_icons.webinars')
                </span>
                <span class="text-sm text-dark-blue font-medium">{{ trans('panel.webinars') }}</span>
            </a>
            ...
            <div class="collapse {{ (request()->is('panel/webinars') or request()->is('panel/webinars/*')) ? 'show' : '' }}" id="webinarCollapse">
                <ul class="sidenav-item-collapse">
                    ...
                    <li class="mt-5 {{ (request()->is('panel/webinars/favorites')) ? 'active' : '' }}">
                        <a href="/panel/webinars/favorites">Whislist</a>
                    </li>
                </ul>
            </div>
        </li>
        ...
        @if(!empty(getFeaturesSettings('upcoming_courses_status')))
            <li class="sidenav-item {{ (request()->is('panel/upcoming_courses') or request()->is('panel/upcoming_courses/*')) ? 'sidenav-item-active' : '' }}">
                <a class="flex items-center" data-toggle="collapse" href="#upcomingCoursesCollapse" role="button" aria-expanded="false" aria-controls="upcomingCoursesCollapse">
                <span class="sidenav-item-icon mr-10">
                    <i data-feather="film" class="img-cover"></i>
                </span>
                    <span class="text-sm text-dark-blue font-medium">{{ trans('update.upcoming_courses') }}</span>
                </a>
        ...
        @if($authUser->isOrganization() or $authUser->isTeacher())
            <li class="sidenav-item {{ (request()->is('panel/bundles') or request()->is('panel/bundles/*')) ? 'sidenav-item-active' : '' }}">
                <a class="flex items-center" data-toggle="collapse" href="#bundlesCollapse" role="button" aria-expanded="false" aria-controls="bundlesCollapse">
                <span class="sidenav-item-icon assign-fill mr-10">
                    @include('web.jiujitsu.panel.includes.sidebar_icons.bundles')
                </span>
                    <span class="text-sm text-dark-blue font-medium">{{ trans('update.bundles') }}</span>
                </a>
        ...
        @if(getFeaturesSettings('webinar_assignment_status'))
            <li class="sidenav-item {{ (request()->is('panel/assignments') or request()->is('panel/assignments/*')) ? 'sidenav-item-active' : '' }}">
                <a class="flex items-center" data-toggle="collapse" href="#assignmentCollapse" role="button" aria-expanded="false" aria-controls="assignmentCollapse">
                <span class="sidenav-item-icon mr-10">
                    @include('web.jiujitsu.panel.includes.sidebar_icons.assignments')
                </span>
                    <span class="text-sm text-dark-blue font-medium">{{ trans('update.assignments') }}</span>
                </a>
        ...
        <li class="sidenav-item {{ (request()->is('panel/meetings') or request()->is('panel/meetings/*')) ? 'sidenav-item-active' : '' }}">
            <a class="flex items-center" data-toggle="collapse" href="#meetingCollapse" role="button" aria-expanded="false" aria-controls="meetingCollapse">
                <span class="sidenav-item-icon mr-10">
                    @include('web.jiujitsu.panel.includes.sidebar_icons.requests')
                </span>
                <span class="text-sm text-dark-blue font-medium">{{ trans('panel.meetings') }}</span>
            </a>
        ...
        <li class="sidenav-item {{ (request()->is('panel/quizzes') or request()->is('panel/quizzes/*')) ? 'sidenav-item-active' : '' }}">
            <a class="flex items-center" data-toggle="collapse" href="#quizzesCollapse" role="button" aria-expanded="false" aria-controls="quizzesCollapse">
                <span class="sidenav-item-icon mr-10">
                    @include('web.jiujitsu.panel.includes.sidebar_icons.quizzes')
                </span>
                <span class="text-sm text-dark-blue font-medium">{{ trans('panel.quizzes') }}</span>
            </a>
        ...
        <li class="sidenav-item {{ (request()->is('panel/certificates') or request()->is('panel/certificates/*')) ? 'sidenav-item-active' : '' }}">
            <a class="flex items-center" data-toggle="collapse" href="#certificatesCollapse" role="button" aria-expanded="false" aria-controls="certificatesCollapse">
                <span class="sidenav-item-icon mr-10">
                    @include('web.jiujitsu.panel.includes.sidebar_icons.certificate')
                </span>
                <span class="text-sm text-dark-blue font-medium">{{ trans('panel.certificates') }}</span>
            </a>
        ...
        @if($authUser->checkCanAccessToStore())
            <li class="sidenav-item {{ (request()->is('panel/store') or request()->is('panel/store/*')) ? 'sidenav-item-active' : '' }}">
                <a class="flex items-center" data-toggle="collapse" href="#storeCollapse" role="button" aria-expanded="false" aria-controls="storeCollapse">
                <span class="sidenav-item-icon assign-fill mr-10">
                    @include('web.jiujitsu.panel.includes.sidebar_icons.store')
                </span>
                    <span class="text-sm text-dark-blue font-medium">{{ trans('update.store') }}</span>
                </a>
                        ...
                        @if($authUser->isOrganization() || $authUser->isTeacher())
                            ...
                            <li class="mt-5 {{ (request()->is('panel/store/sales')) ? 'active' : '' }}">
                                ...
                                @if($sellerProductOrderWaitingDeliveryCount > 0)
                                    <span class="d-inline-flex items-center justify-center font-medium ml-15 panel-sidebar-store-sales-circle-badge">{{ $sellerProductOrderWaitingDeliveryCount }}</span>
                                @endif
                            </li>
        ...
        <li class="sidenav-item {{ (request()->is('panel/financial') or request()->is('panel/financial/*')) ? 'sidenav-item-active' : '' }}">
            <a class="flex items-center" data-toggle="collapse" href="#financialCollapse" role="button" aria-expanded="false" aria-controls="financialCollapse">
                <span class="sidenav-item-icon mr-10">
                    @include('web.jiujitsu.panel.includes.sidebar_icons.financial')
                </span>
                <span class="text-sm text-dark-blue font-medium">{{ trans('panel.financial') }}</span>
            </a>
        ...
        <li class="sidenav-item {{ (request()->is('panel/support') or request()->is('panel/support/*')) ? 'sidenav-item-active' : '' }}">
            <a class="flex items-center" data-toggle="collapse" href="#supportCollapse" role="button" aria-expanded="false" aria-controls="supportCollapse">
                <span class="sidenav-item-icon assign-fill mr-10">
                    @include('web.jiujitsu.panel.includes.sidebar_icons.support')
                </span>
                <span class="text-sm text-dark-blue font-medium">{{ trans('panel.support') }}</span>
            </a>
        ...
        @if(!$authUser->isUser() or (!empty($referralSettings) and $referralSettings['status'] and $authUser->affiliate) or (!empty(getRegistrationBonusSettings('status')) and $authUser->enable_registration_bonus))
            <li class="sidenav-item {{ (request()->is('panel/marketing') or request()->is('panel/marketing/*')) ? 'sidenav-item-active' : '' }}">
                <a class="flex items-center" data-toggle="collapse" href="#marketingCollapse" role="button" aria-expanded="false" aria-controls="marketingCollapse">
                <span class="sidenav-item-icon mr-10">
                    @include('web.jiujitsu.panel.includes.sidebar_icons.marketing')
                </span>
                    <span class="text-sm text-dark-blue font-medium">{{ trans('panel.marketing') }}</span>
                </a>
        ...
        @if(getFeaturesSettings('forums_status'))
            <li class="sidenav-item {{ (request()->is('panel/forums') or request()->is('panel/forums/*')) ? 'sidenav-item-active' : '' }}">
                <a class="flex items-center" data-toggle="collapse" href="#forumsCollapse" role="button" aria-expanded="false" aria-controls="forumsCollapse">
                <span class="sidenav-item-icon assign-fill mr-10">
                    @include('web.jiujitsu.panel.includes.sidebar_icons.forums')
                </span>
                    <span class="text-sm text-dark-blue font-medium">{{ trans('update.forums') }}</span>
                </a>
        ...
        @if($authUser->isTeacher())
            <li class="sidenav-item {{ (request()->is('panel/blog') or request()->is('panel/blog/*')) ? 'sidenav-item-active' : '' }}">
                <a class="flex items-center" data-toggle="collapse" href="#blogCollapse" role="button" aria-expanded="false" aria-controls="blogCollapse">
                <span class="sidenav-item-icon assign-fill mr-10">
                    @include('web.jiujitsu.panel.includes.sidebar_icons.blog')
                </span>
                    <span class="text-sm text-dark-blue font-medium">{{ trans('update.articles') }}</span>
                </a>
        ...
        @if($authUser->isOrganization() || $authUser->isTeacher())
            <li class="sidenav-item {{ (request()->is('panel/noticeboard*') or request()->is('panel/course-noticeboard*')) ? 'sidenav-item-active' : '' }}">
                <a class="flex items-center" data-toggle="collapse" href="#noticeboardCollapse" role="button" aria-expanded="false" aria-controls="noticeboardCollapse">
                <span class="sidenav-item-icon mr-10">
                    @include('web.jiujitsu.panel.includes.sidebar_icons.noticeboard')
                </span>

                    <span class="text-sm text-dark-blue font-medium">{{ trans('panel.noticeboard') }}</span>
                </a>
        ...
        @if(!empty($rewardSetting) and $rewardSetting['status'] == '1')
            <li class="sidenav-item {{ (request()->is('panel/rewards')) ? 'sidenav-item-active' : '' }}">
                <a href="/panel/rewards" class="flex items-center">
                <span class="sidenav-item-icon assign-strock mr-10">
                    @include('web.jiujitsu.panel.includes.sidebar_icons.rewards')
                </span>
                    <span class="text-sm text-dark-blue font-medium">{{ trans('update.rewards') }}</span>
                </a>
            </li>
        @endif
        ...
        @if($authUser->checkAccessToAIContentFeature())
            <li class="sidenav-item {{ (request()->is('panel/ai-contents')) ? 'sidenav-item-active' : '' }}">
                <a href="/panel/ai-contents" class="flex items-center">
                <span class="sidenav-item-icon assign-strock mr-10">
                    @include('web.jiujitsu.panel.includes.sidebar_icons.ai_contents')
                </span>
                    <span class="text-sm text-dark-blue font-medium">{{ trans('update.ai_contents') }}</span>
                </a>
            </li>
        @endif

        <li class="sidenav-item {{ (request()->is('panel/notifications')) ? 'sidenav-item-active' : '' }}">
            <a href="/panel/notifications" class="flex items-center">
            <span class="sidenav-notification-icon sidenav-item-icon mr-10">
                    @include('web.jiujitsu.panel.includes.sidebar_icons.notifications')
                </span>
                <span class="text-sm text-dark-blue font-medium">{{ trans('panel.notifications') }}</span>
            </a>
        </li>

        <li class="sidenav-item {{ (request()->is('panel/setting')) ? 'sidenav-item-active' : '' }}">
            <a href="/panel/setting" class="flex items-center">
                <span class="sidenav-setting-icon sidenav-item-icon mr-10">
                    @include('web.jiujitsu.panel.includes.sidebar_icons.setting')
                </span>
                <span class="text-sm text-dark-blue font-medium">{{ trans('panel.settings') }}</span>
            </a>
        </li>

        @if($authUser->isTeacher() or $authUser->isOrganization())
            <li class="sidenav-item ">
                <a href="{{ $authUser->getProfileUrl() }}" class="flex items-center">
                <span class="sidenav-item-icon assign-strock mr-10">
                    <i data-feather="user" stroke="#1f3b64" stroke-width="1.5" width="24" height="24" class="mr-10 webinar-icon"></i>
                </span>
                    <span class="text-sm text-dark-blue font-medium">{{ trans('public.my_profile') }}</span>
                </a>
            </li>
        @endif
        ...
        <li class="sidenav-item">
            <a href="/logout" class="flex items-center">
                <span class="sidenav-logout-icon sidenav-item-icon mr-10">
                    @include('web.jiujitsu.panel.includes.sidebar_icons.logout')
                </span>
                <span class="text-sm text-dark-blue font-medium">{{ trans('panel.log_out') }}</span>
            </a>
        </li>
    </ul>

    @if(!empty($getPanelSidebarSettings) and !empty($getPanelSidebarSettings['background']))
        <div class="sidebar-create-class hidden d-mblock">
            <a href="{{ !empty($getPanelSidebarSettings['link']) ? $getPanelSidebarSettings['link'] : '' }}" class="sidebar-create-class-btn block text-right p-5">
