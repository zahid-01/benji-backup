...
@section('content')
    <div class="">
        ...
        <form method="post" action="/panel/upcoming_courses/{{ !empty($upcomingCourse) ? $upcomingCourse->id .'/update' : 'store' }}" id="upcomingCourseForm" class="webinar-form">
            @include('web.jiujitsu.panel.upcoming_courses.create_includes.progress')
            ...
            @if($currentStep == 1)
                @include('web.jiujitsu.panel.upcoming_courses.create_includes.step_1')
            @elseif(!empty($upcomingCourse))
                @include('web.jiujitsu.panel.upcoming_courses.create_includes.step_'.$currentStep)
            @endif

        </form>


        <div class="create-webinar-footer flex flex-col flex-md-row items-center justify-between mt-4 pt-15 border-top">
            ...
            <div class="mt-4 mt-md-0">
                ...
                @if(!empty($upcomingCourse) and $upcomingCourse->creator_id == $authUser->id)
                    <a href="/panel/upcoming_courses/{{ $upcomingCourse->id }}/delete?redirect_to=/panel/upcoming_courses" class="delete-action webinar-actions btn btn-sm bg-red-600 text-white mt-4 mt-md-0">{{ trans('public.delete') }}</a>
                @endif
            </div>
        </div>
    </div>
