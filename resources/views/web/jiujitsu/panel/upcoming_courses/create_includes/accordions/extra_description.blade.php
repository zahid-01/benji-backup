<li data-id="{{ !empty($extraDescription) ? $extraDescription->id :'' }}" class="accordion-row bg-white rounded-sm panel-shadow mt-4 py-15 py-lg-30 px-10 px-lg-20">
    <div class="flex items-center justify-between " role="tab" id="{{ $extraDescriptionType }}_{{ !empty($extraDescription) ? $extraDescription->id :'record' }}">
        <div class="font-bold text-dark-blue" href="#collapseExtraDescription{{ !empty($extraDescription) ? $extraDescription->id :'record' }}" aria-controls="collapseExtraDescription{{ !empty($extraDescription) ? $extraDescription->id :'record' }}" data-parent="#{{ $extraDescriptionParentAccordion }}" role="button" data-toggle="collapse" aria-expanded="true">
            ...
            @if(!empty($extraDescription))
                <div class="btn-group dropdown table-actions mr-15">
                    ...
                    <div class="dropdown-menu">
                        <a href="/panel/webinar-extra-description/{{ $extraDescription->id }}/delete" class="delete-action btn btn-sm btn-ghost">{{ trans('public.delete') }}</a>
                    </div>
                </div>
            @endif
