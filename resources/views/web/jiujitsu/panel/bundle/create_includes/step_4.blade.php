                ...
                @if(!empty($bundle->bundleWebinars) and count($bundle->bundleWebinars))
                    <ul class="draggable-lists" data-order-table="bundle_webinars">
                        @foreach($bundle->bundleWebinars as $bundleWebinarRow)
                            @include('web.jiujitsu.panel.bundle.create_includes.accordions.bundle-webinars',['bundle' => $bundle, 'bundleWebinar' => $bundleWebinarRow])
                        @endforeach
                    </ul>
                @else
...
<div id="newBundleWebinarForm" class="hidden">
    @include('web.jiujitsu.panel.bundle.create_includes.accordions.bundle-webinars',['bundle' => $bundle])
</div>
