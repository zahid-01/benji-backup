...
@section('content')
    <div class="">
        ...
        <form method="post" action="/panel/webinars/{{ !empty($webinar) ? $webinar->id .'/update' : 'store' }}" id="webinarForm" class="webinar-form">
            @include('web.jiujitsu.panel.webinar.create_includes.progress')
            ...
            @if($currentStep == 1)
                @include('web.jiujitsu.panel.webinar.create_includes.step_1')
            @elseif(!empty($webinar))
                @include('web.jiujitsu.panel.webinar.create_includes.step_'.$currentStep)
            @endif
        ...
        <div class="create-webinar-footer flex flex-col flex-md-row items-center justify-between mt-4 pt-15 border-top">
            ...
            <div class="mt-4 mt-md-0">
                ...
                @if(!empty($webinar) and $webinar->creator_id == $authUser->id)
                    <a href="/panel/webinars/{{ $webinar->id }}/delete?redirect_to=/panel/webinars" class="delete-action webinar-actions btn btn-sm bg-red-600 text-white mt-4 mt-md-0">{{ trans('public.delete') }}</a>
                @endif
            </div>
        </div>
    </div>
