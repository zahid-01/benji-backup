    ...
    <div class="col-12 col-md-6">
        ...
        @if($authUser->isOrganization() and $authUser->id == $webinar->creator_id)
            <div class="form-group mt-15">
                ...
                @enderror
                <p class="text-sm text-slate-500 mt-5">- {{ trans('update.organization_price_hint') }}</p>
            </div>
        @endif
    </div>
</div>
                ...
                @if(!empty($webinar->tickets) and count($webinar->tickets))
                    <ul class="draggable-lists" data-order-table="tickets">
                        @foreach($webinar->tickets as $ticketInfo)
                            @include('web.jiujitsu.panel.webinar.create_includes.accordions.ticket',['webinar' => $webinar,'ticket' => $ticketInfo])
                        @endforeach
                    </ul>
                @else
...
<div id="newTicketForm" class="hidden">
    @include('web.jiujitsu.panel.webinar.create_includes.accordions.ticket',['webinar' => $webinar])
</div>
