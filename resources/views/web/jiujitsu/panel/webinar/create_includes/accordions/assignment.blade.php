<li data-id="{{ !empty($chapterItem) ? $chapterItem->id :'' }}" class="accordion-row bg-white rounded-sm border border-gray300 mt-4 py-15 py-lg-30 px-10 px-lg-20">
    <div class="flex items-center justify-between " role="tab" id="file_{{ !empty($assignment) ? $assignment->id :'record' }}">
        <div class="flex items-center" href="#collapseFile{{ !empty($assignment) ? $assignment->id :'record' }}" aria-controls="collapseFile{{ !empty($assignment) ? $assignment->id :'record' }}" data-parent="#chapterContentAccordion{{ !empty($chapter) ? $chapter->id :'' }}" role="button" data-toggle="collapse" aria-expanded="true">
            <span class="chapter-icon chapter-content-icon mr-10">
                <i data-feather="feather" class=""></i>
            </span>

            <div class="font-bold text-dark-blue block">{{ !empty($assignment) ? $assignment->title . ($assignment->accessibility == 'free' ? " (". trans('public.free') .")" : '') : trans('update.add_new_assignments') }}</div>
        </div>
        ...
        <div class="flex items-center">
            ...
            @if(!empty($assignment))
                <button type="button" data-item-id="{{ $assignment->id }}" data-item-type="{{ \App\Models\WebinarChapterItem::$chapterAssignment }}" data-chapter-id="{{ !empty($chapter) ? $chapter->id : '' }}" class="js-change-content-chapter btn btn-sm btn-ghost text-slate-500 mr-10">
            ...
            @if(!empty($assignment))
                <a href="/panel/assignments/{{ $assignment->id }}/delete" class="delete-action btn btn-sm btn-ghost text-slate-500">
                ...
                <div class="row">
                    <div class="col-12 col-lg-6">
                        ...
                        <div class="js-assignment-attachments-items form-group mt-15">
                            ...
                            <div class="assignment-attachments-main-row js-ajax-attachments relative">
                                ...
                                <button type="button" class="btn bg-red-600 text-white btn-sm assignment-attachments-remove-btn hidden">
                            ...
                            @if(!empty($assignment) and !empty($assignment->attachments) and count($assignment->attachments))
                                @foreach($assignment->attachments as $attachment)
                                    <div class="js-ajax-attachments relative">
                                        <div class="mt-10 p-10 border rounded">
                                            <div class="mb-10">
                                                <label class="input-label">{{ trans('public.title') }}</label>
                                        ...
                                        <button type="button" class="btn bg-red-600 text-white btn-sm assignment-attachments-remove-btn">
                                            <i data-feather="x" width="18" height="18" class="text-white"></i>
                                        </button>
                                    </div>
                                @endforeach
                        ...
                        @if(getFeaturesSettings('sequence_content_status'))
                            ...
                            <div class="js-sequence-content-inputs pl-5 {{ (!empty($assignment) and ($assignment->check_previous_parts or !empty($assignment->access_after_day))) ? '' : 'hidden' }}">
                                <div class="form-group">
                                    <div class="flex items-center justify-between">
