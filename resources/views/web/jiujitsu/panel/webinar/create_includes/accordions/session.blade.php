...
<li data-id="{{ !empty($chapterItem) ? $chapterItem->id :'' }}" class="accordion-row bg-white rounded-sm border border-gray300 mt-4 py-15 py-lg-30 px-10 px-lg-20">
    <div class="flex items-center justify-between " role="tab" id="session_{{ !empty($session) ? $session->id :'record' }}">
        <div class="flex items-center" href="#collapseSession{{ !empty($session) ? $session->id :'record' }}" aria-controls="collapseSession{{ !empty($session) ? $session->id :'record' }}" data-parent="#chapterContentAccordion{{ !empty($chapter) ? $chapter->id :'' }}" role="button" data-toggle="collapse" aria-expanded="true">
            <span class="chapter-icon chapter-content-icon mr-10">
                <i data-feather="file-text" class=""></i>
            </span>

            <div class="font-bold text-dark-blue block">{{ !empty($session) ? $session->title : trans('public.add_new_sessions') }}</div>
        </div>
        ...
        <div class="flex items-center">
            ...
            @if(!empty($session))
                <button type="button" data-item-id="{{ $session->id }}" data-item-type="{{ \App\Models\WebinarChapterItem::$chapterSession }}" data-chapter-id="{{ !empty($chapter) ? $chapter->id : '' }}" class="js-change-content-chapter btn btn-sm btn-ghost text-slate-500 mr-10">
            ...
            @if(!empty($session))
                <a href="/panel/sessions/{{ $session->id }}/delete" class="delete-action btn btn-sm btn-ghost text-slate-500">
        ...
        <div class="panel-collapse text-slate-500">
            <div class="js-content-form session-form" data-action="/panel/sessions/{{ !empty($session) ? $session->id . '/update' : 'store' }}">
                ...
                <div class="form-group">
                    ...
                    <div class="js-zoom-not-complete-alert mt-10 text-danger hidden">
                ...
                <div class="row">
                    <div class="col-12 col-lg-6">
                        ...
                        <div class="form-group js-api-secret {{ (!empty($session) and in_array($session->session_api, ['zoom', 'agora', 'jitsi'])) ? 'hidden' :'' }}">
                        ...
                        <div class="form-group js-moderator-secret {{ (empty($session) or $session->session_api != 'big_blue_button') ? 'hidden' :'' }}">
                        ...
                        <div class="form-group js-local-link {{ (!empty($session) and in_array($session->session_api, ['agora', 'jitsi'])) ? 'hidden' : '' }}">
                        ...
                        <div class="js-agora-chat-and-rec  {{ (empty($session) or $session->session_api !== 'agora') ? 'hidden' : '' }}">
                        ...
                        @if(getFeaturesSettings('sequence_content_status'))
                            ...
                            <div class="js-sequence-content-inputs pl-5 {{ (!empty($session) and ($session->check_previous_parts or !empty($session->access_after_day))) ? '' : 'hidden' }}">
                                <div class="form-group">
                                    <div class="flex items-center justify-between">
