<li data-id="{{ !empty($chapterItem) ? $chapterItem->id :'' }}" class="accordion-row bg-white rounded-sm border border-gray300 mt-4 py-15 py-lg-30 px-10 px-lg-20">
    <div class="flex items-center justify-between " role="tab" id="text_lesson_{{ !empty($textLesson) ? $textLesson->id :'record' }}">
        <div class="flex items-center" href="#collapseTextLesson{{ !empty($textLesson) ? $textLesson->id :'record' }}" aria-controls="collapseTextLesson{{ !empty($textLesson) ? $textLesson->id :'record' }}" data-parent="#chapterContentAccordion{{ !empty($chapter) ? $chapter->id :'' }}" role="button" data-toggle="collapse" aria-expanded="true">
            <span class="chapter-icon chapter-content-icon mr-10">
                <i data-feather="file-text" class=""></i>
            </span>

            <div class="font-bold text-dark-blue block">{{ !empty($textLesson) ? $textLesson->title . ($textLesson->accessibility == 'free' ? " (". trans('public.free') .")" : '') : trans('public.add_new_test_lesson') }}</div>
        </div>
        ...
        <div class="flex items-center">
            ...
            @if(!empty($textLesson))
                <button type="button" data-item-id="{{ $textLesson->id }}" data-item-type="{{ \App\Models\WebinarChapterItem::$chapterTextLesson }}" data-chapter-id="{{ !empty($chapter) ? $chapter->id : '' }}" class="js-change-content-chapter btn btn-sm btn-ghost text-slate-500 mr-10">
            ...
            @if(!empty($textLesson))
                <a href="/panel/text-lesson/{{ $textLesson->id }}/delete" class="delete-action btn btn-sm btn-ghost text-slate-500">
                ...
                <div class="row">
                    <div class="col-12 col-lg-6">
                        ...
                        <div class="form-group">
                            ...
                            <div class="flex items-center js-ajax-accessibility">
                                <div class="custom-control custom-radio">
                                    ...
                                    <label class="custom-control-label text-sm cursor-pointer" for="accessibilityRadio1_{{ !empty($textLesson) ? $textLesson->id : 'record' }}">{{ trans('public.free') }}</label>
                                </div>
                                ...
                                <div class="custom-control custom-radio ml-15">
                                    ...
                                    <label class="custom-control-label text-sm cursor-pointer" for="accessibilityRadio2_{{ !empty($textLesson) ? $textLesson->id : 'record' }}">{{ trans('public.paid') }}</label>
                                </div>
                            </div>
                            <div class="invalid-feedback"></div>
                        </div>
                        ...
                        <div class="form-group">
                            <label class="input-label block">{{ trans('public.attachments') }}</label>
                    ...
                    <div class="col-12">
                        <div class="form-group">
                            ...
                            <div class="content-summernote js-ajax-file_path">
                                ...
                                <textarea name="ajax[{{ !empty($textLesson) ? $textLesson->id : 'new' }}][content]" class="js-hidden-content-summernote {{ !empty($textLesson) ? 'js-hidden-content-'.$textLesson->id : '' }} hidden">{{ !empty($textLesson) ? $textLesson->content : '' }}</textarea>
                            </div>
                            <div class="invalid-feedback"></div>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-lg-6">
                        ...
                        @if(getFeaturesSettings('sequence_content_status'))
                            ...
                            <div class="js-sequence-content-inputs pl-5 {{ (!empty($textLesson) and ($textLesson->check_previous_parts or !empty($textLesson->access_after_day))) ? '' : 'hidden' }}">
                                <div class="form-group">
                                    <div class="flex items-center justify-between">
