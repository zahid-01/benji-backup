@if(!empty($file) and $file->storage == 'upload_archive')
    @include('web.jiujitsu.panel.webinar.create_includes.accordions.new_interactive_file',['file' => $file])
@else
    <li data-id="{{ !empty($chapterItem) ? $chapterItem->id :'' }}" class="accordion-row bg-white rounded-sm border border-gray300 mt-4 py-15 py-lg-30 px-10 px-lg-20">
        <div class="flex items-center justify-between " role="tab" id="file_{{ !empty($file) ? $file->id :'record' }}">
            <div class="flex items-center" href="#collapseFile{{ !empty($file) ? $file->id :'record' }}" aria-controls="collapseFile{{ !empty($file) ? $file->id :'record' }}" data-parent="#chapterContentAccordion{{ !empty($chapter) ? $chapter->id :'' }}" role="button" data-toggle="collapse" aria-expanded="true">
            <span class="chapter-icon chapter-content-icon mr-10">
                <i data-feather="{{ !empty($file) ? $file->getIconByType() : 'file' }}" class=""></i>
            </span>

                <div class="font-bold text-dark-blue block">{{ !empty($file) ? $file->title . ($file->accessibility == 'free' ? " (". trans('public.free') .")" : '') : trans('public.add_new_files') }}</div>
            </div>
            ...
            <div class="flex items-center">
                ...
                @if(!empty($file))
                    <button type="button" data-item-id="{{ $file->id }}" data-item-type="{{ \App\Models\WebinarChapterItem::$chapterFile }}" data-chapter-id="{{ !empty($chapter) ? $chapter->id : '' }}" class="js-change-content-chapter btn btn-sm btn-ghost text-slate-500 mr-10">
                ...
                @if(!empty($file))
                    <a href="/panel/files/{{ $file->id }}/delete" class="delete-action btn btn-sm btn-ghost text-slate-500">
                ...
                <div class="js-content-form file-form" data-action="/panel/files/{{ !empty($file) ? $file->id . '/update' : 'store' }}">
                    ...
                    <div class="row">
                        <div class="col-12 col-lg-6">
                            ...
                            <div class="form-group">
                                ...
                                <div class="flex items-center js-ajax-accessibility">
                                    <div class="custom-control custom-radio">
                                        ...
                                        <label class="custom-control-label text-sm cursor-pointer" for="accessibilityRadio1_{{ !empty($file) ? $file->id : 'record' }}">{{ trans('public.free') }}</label>
                                    </div>
                                    ...
                                    <div class="custom-control custom-radio ml-15">
                                        ...
                                        <label class="custom-control-label text-sm cursor-pointer" for="accessibilityRadio2_{{ !empty($file) ? $file->id : 'record' }}">{{ trans('public.paid') }}</label>
                                    </div>
                                </div>
                            ...
                            <div class="form-group js-file-path-input {{ (!empty($file) and $file->storage == 's3') ? 'hidden' : '' }}">
                            ...
                            <div class="form-group js-s3-file-path-input {{ (!empty($file) and $file->storage == 's3') ? '' : 'hidden' }}">
                            ...
                            <div class="row form-group js-file-type-volume hidden">
                                <div class="col-6">
                            ...
                            <div class="js-online_viewer-input form-group mt-4">
                            ...
                            <div class="js-downloadable-input form-group mt-4">
                                <div class="flex items-center justify-between">
                            ...
                            @if(getFeaturesSettings('sequence_content_status'))
                                ...
                                <div class="js-sequence-content-inputs pl-5 {{ (!empty($file) and ($file->check_previous_parts or !empty($file->access_after_day))) ? '' : 'hidden' }}">
                                    <div class="form-group">
                                        <div class="flex items-center justify-between">
