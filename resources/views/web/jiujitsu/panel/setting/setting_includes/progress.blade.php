...
<div class="webinar-progress block lg:flex items-center p-15 panel-shadow bg-white rounded-sm">
    ...
    @foreach($progressSteps as $key => $step)
        <div class="progress-item flex items-center">
            <a href="@if(!empty($organization_id)) /panel/manage/{{ $user_type ?? 'instructors' }}/{{ $user->id }}/edit/step/{{ $key }} @else /panel/setting/step/{{ $key }} @endif" class="progress-icon p-10 flex items-center justify-center rounded-full {{ $key == $currentStep ? 'active' : '' }}" data-toggle="tooltip" data-placement="top" title="{{ trans($step['lang']) }}">
                <img src="/assets/default/img/icons/{{ $step['icon'] }}.svg" class="img-cover" alt="">
            </a>

            <div class="ml-10 {{ $key == $currentStep ? '' : 'lg:hidden' }}">
                <h4 class="font-16 text-black font-bold">{{ trans($step['lang']) }}</h4>
            </div>
        </div>
    @endforeach
</div>
