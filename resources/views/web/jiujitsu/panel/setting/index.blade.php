...
@section('content')
    @if(empty($new_user))
        @include('web.jiujitsu.panel.setting.setting_includes.progress')
    @endif

    <form method="post" id="userSettingForm" class="mt-6" action="{{ (!empty($new_user)) ? '/panel/manage/'. $user_type .'/new' : '/panel/setting' }}">
        {{ csrf_field() }}
        ...
        @if(!empty($new_user) or (!empty($currentStep) and $currentStep == 1))
            @include('web.jiujitsu.panel.setting.setting_includes.basic_information')
        @endif

        @if(empty($new_user) and !empty($currentStep))
            @switch($currentStep)
                @case(2)
                @include('web.jiujitsu.panel.setting.setting_includes.image')
                @break

                @case(3)
                @include('web.jiujitsu.panel.setting.setting_includes.about')
                @break

                @case(4)
                @include('web.jiujitsu.panel.setting.setting_includes.education')
                @break

                @case(5)
                @include('web.jiujitsu.panel.setting.setting_includes.experiences')
                @break

                @case(6)
                @include('web.jiujitsu.panel.setting.setting_includes.occupations')
                @break

                @case(7)
                @include('web.jiujitsu.panel.setting.setting_includes.identity_and_financial')
                @break

                @case(8)
                @if(!$user->isUser())
                        @include('web.jiujitsu.panel.setting.setting_includes.zoom_api')
                @else
                        @include('web.jiujitsu.panel.setting.setting_includes.settings')
                @endif
                @break
    ...
    <div class="create-webinar-footer flex items-center justify-between mt-4 pt-15 border-top">
        ...
        <div class="flex items-center">
            @if(empty($new_user) and empty($edit_new_user))
                <a href="/panel/setting/deleteAccount" class="delete-action btn btn-sm bg-red-600 text-white" data-confirm="{{ trans('update.delete_account_modal_confirm_btn_text') }}" data-title="{{ trans('update.delete_account_modal_hint') }}">{{ trans('update.delete_account') }}</a>
            @endif
