    ...
    <section>
        ...
        <div class="panel-section-card py-20 px-4 mt-4">
            <form action="/panel/marketing/special_offers/store" method="post" class="row">
                ...
                <div class="col-12 col-lg-6">
                    <div class="row">
                        ...
                        <div class="col-12 col-lg-4">
                            <div class="form-group">
                                <label class="input-label block">{{ trans('panel.webinar') }}</label>
                ...
                <div class="col-12 col-lg-1 flex items-center justify-end">
    ...
    @if(!empty($specialOffers) and $specialOffers->count() > 0)
        ...
        <section class="mt-8">
            <div class="flex align-items-start align-items-md-center justify-between flex-col flex-md-row">
                ...
                <form action="" method="get" class="flex items-center flex-row-reverse flex-md-row justify-content-start justify-content-md-center mt-4 mt-md-0">
                    <label class="cursor-pointer mb-0 mr-10 text-slate-500 text-sm font-medium" for="activeDiscountsSwitch">{{ trans('panel.show_only_active_discounts') }}</label>
                    ...
                    <div class="col-12 ">
                        <div class="table-responsive">
                            <table class="table custom-table text-center">
                                <thead>
                                <tr>
                                    <th class="text-left text-slate-500">{{ trans('panel.name') }}</th>
                                    <th class="text-left text-slate-500">{{ trans('panel.webinar') }}</th>
                                    <th class="text-center text-slate-500">{{ trans('panel.amount') }}</th>
                                    <th class="text-center text-slate-500">{{ trans('public.status') }}</th>
                                    <th class="text-center text-slate-500">{{ trans('public.from') }}</th>
                                    <th class="text-center text-slate-500">{{ trans('public.to') }}</th>
                                    <th></th>
                                </tr>
                                </thead>
                                <tbody>
                                @foreach($specialOffers as $specialOffer)
                                    <tr>
                                        <td class="text-left align-middle text-dark-blue font-medium">{{ $specialOffer->name }}</td>
                                        <td class="text-left align-middle">
                                            <a href="{{ $specialOffer->webinar->getUrl() }}" class="text-dark-blue font-medium" target="_blank">{{ $specialOffer->webinar->title }}</a>
                                        </td>
                                        <td class="align-middle">
                                            <span class="text-dark-blue font-medium">{{ $specialOffer->percent }}%</span>
                                        </td>
                                        <td class="align-middle">
                                            @switch($specialOffer->status)
                                                @case(\App\Models\SpecialOffer::$active)
                                                <span class="text-primary font-medium">{{ trans('public.active') }}</span>

                                                @break
                                                @case(\App\Models\SpecialOffer::$inactive)
                                                <span class="text-warning font-medium">{{ trans('public.inactive') }}</span>
                                                @break
                                            @endswitch
                                        </td>
                                        <td class="align-middle text-dark-blue font-medium">{{ dateTimeFormat($specialOffer->from_date, 'j M Y | H:i') }}</td>
                                        <td class="align-middle text-dark-blue font-medium">{{ dateTimeFormat($specialOffer->to_date, 'j M Y | H:i') }}</td>
                                                ...
                                                <div class="btn-group dropdown table-actions">
                                                    ...
                                                    <div class="dropdown-menu">
                                                        <a href="/panel/marketing/special_offers/{{ $specialOffer->id }}/disable" type="button"
                                                           class="delete-action btn-ghost">{{ trans('public.disable') }}</a>
                                                    </div>
                                                </div>
                                            @endif
                                        </td>
