...
@section('content')
    ...
    @if(!empty($accounting))
        <div class="flex items-center mb-20 p-15 success-transparent-alert">
            <div class="success-transparent-alert__icon flex items-center justify-center">
                <i data-feather="credit-card" width="18" height="18" class=""></i>
            </div>
            <div class="ml-10">
                <div class="text-sm font-bold ">{{ trans('update.you_got_the_bonus') }}</div>
                <div class="text-sm ">{{ trans('update.your_registration_bonus_was_unlocked_on_date',['date' => dateTimeFormat($accounting->created_at, 'j M Y')]) }}</div>
            </div>
        </div>
    @endif
            ...
            <div class="row">
                ...
                <div class="col-4 flex items-center justify-center">
                    <div class="flex flex-col items-center text-center">
                        ...
                        <strong class="text-3xl text-dark-blue font-bold mt-5">{{ handlePrice($registrationBonusSettings['registration_bonus_amount'] ?? 0) }}</strong>
                        <span class="font-16 text-slate-500 font-medium">{{ trans('update.registration_bonus') }}</span>
                    </div>
                </div>
                ...
                <div class="col-4 flex items-center justify-center">
                    <div class="flex flex-col items-center text-center">
                        ...
                        <strong class="font-36 font-bold mt-5 {{ !empty($accounting) ? 'text-primary' : 'text-danger' }}">{{ !empty($accounting) ? trans('update.unlocked') : trans('update.locked') }}</strong>
                        <span class="font-16 text-slate-500 font-medium">{{ trans('update.bonus_status') }}</span>
                    </div>
                </div>
                ...
                <div class="col-4 flex items-center justify-center">
                    <div class="flex flex-col items-center text-center">
                        ...
                        <strong class="text-3xl text-dark-blue font-bold mt-5">{{ !empty($accounting) ? dateTimeFormat($accounting->created_at, 'j M Y') : '-' }}</strong>
                        <span class="font-16 text-slate-500 font-medium">{{ trans('update.bonus_date') }}</span>
                    </div>
                </div>
            ...
            <div class="col-12 col-md-6 mt-25">
                <div class="p-15 rounded-sm panel-shadow bg-white h-100">
                    <div class="row">
                        ...
                        <div class="col-7">
                            <h4 class="font-16 font-bold">{{ trans('update.bonus_status') }}</h4>

                            <p class="mt-10 text-sm font-medium text-slate-500">{{ trans('update.your_bonus_is_locked_To_unlock_the_bonus_please_check_the_following_statuses') }}:</p>

                            @if(!empty($registrationBonusSettings['number_of_referred_users']))
                                <div class="flex items-center relative mt-15 p-15 border border-gray200 rounded-lg">
                                    <div class="bonus-status-pie-charts">
                                        <canvas id="bonusStatusReferredUsersChart" height="40"></canvas>
                                    </div>

                                    <div class="ml-2">
                                        <span class="block text-sm font-bold text-slate-500">{{ trans('update.referred_users') }}</span>
                                        <span class="block text-sm text-slate-500">{{ $bonusStatusReferredUsersChart['complete'] == 0 ? trans('update.you_havent_referred_any_users') : trans('update.you_referred_count_users_to_the_platform',['count' => "{$bonusStatusReferredUsersChart['referred_users']}/{$registrationBonusSettings['number_of_referred_users']}"]) }}</span>
                                    </div>
        ...
        @if(!empty($registrationBonusTermsSettings) and !empty($registrationBonusTermsSettings['items']))
            <div class="mt-25 {{ (!empty($registrationBonusSettings['number_of_referred_users']) or !empty($registrationBonusSettings['purchase_amount_for_unlocking_bonus'])) ? 'col-12 col-md-6' : 'col-12' }}">
                <div class="p-15 rounded-sm panel-shadow bg-white h-100">
                    <div class="row">
                        <div class="col-7">
                            <h4 class="font-16 font-bold mb-20">{{ trans('update.how_to_get_bonus') }}</h4>
                            ...
                            @foreach($registrationBonusTermsSettings['items'] as $termItem)
                                @if(!empty($termItem['icon']) and !empty($termItem['title']) and !empty($termItem['description']))
                                    <div class="how-to-get-bonus-items flex align-items-start">
                                        <div class="icon-box flex items-center justify-center">
                                            <img src="{{ $termItem['icon'] }}" alt="{{ $termItem['title'] }}" width="16" height="16">
                                        </div>
                                        <div class="ml-10">
                                            <span class="block text-sm font-bold text-dark">{{ $termItem['title'] }}</span>
                                            <span class="block text-sm font-medium text-slate-500 mt-5">{{ $termItem['description'] }}</span>
                                        </div>
                                    </div>
                                @endif
                            @endforeach
    ...
    @if($checkReferralUserCount)
        <section class="mt-25">
                                            ...
                                            @if($purchaseAmountCount)
                                                <td>
                                                    @if((!empty($registrationBonusSettings['purchase_amount_for_unlocking_bonus']) and $user->totalPurchase >= $registrationBonusSettings['purchase_amount_for_unlocking_bonus']) or (empty($registrationBonusSettings['purchase_amount_for_unlocking_bonus']) and $user->totalPurchase > 0))
                                                        <span class="font-16 font-medium text-primary">{{ trans('update.reached') }}</span>
                                                    @else
                                                        <span class="font-16 font-medium text-dark">{{ trans('update.not_reached') }}</span>
                                                    @endif
                                                </td>
                                            @endif
            ...
            @else
                <div class="no-result my-50 flex items-center justify-center flex-col">
