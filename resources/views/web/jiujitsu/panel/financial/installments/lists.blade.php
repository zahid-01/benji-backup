...
@section('content')
    ...
    @if(!empty($overdueInstallmentsCount) and $overdueInstallmentsCount > 0)
        <div class="flex items-center mb-20 p-15 danger-transparent-alert">
            ...
            <div class="ml-10">
                ...
                <div class="text-sm ">{{ trans('update.you_have_count_overdue_installments_please_pay_them_to_avoid_restrictions_and_negative_effects_on_your_account',['count' => $overdueInstallmentsCount]) }}</div>
            </div>
        </div>
    @endif
            ...
            <div class="row">
                <div class="col-6 col-md-3 mt-6 mt-md-0 flex items-center justify-center">
                    <div class="flex flex-col items-center text-center">
                        ...
                        <strong class="text-3xl text-dark-blue font-bold mt-5">{{ $openInstallmentsCount }}</strong>
                        <span class="font-16 text-slate-500 font-medium">{{ trans('update.open_installments') }}</span>
                    </div>
                </div>
                ...
                <div class="col-6 col-md-3 mt-6 mt-md-0 flex items-center justify-center">
                    <div class="flex flex-col items-center text-center">
                        ...
                        <strong class="text-3xl text-dark-blue font-bold mt-5">{{ $pendingVerificationCount }}</strong>
                        <span class="font-16 text-slate-500 font-medium">{{ trans('update.pending_verification') }}</span>
                    </div>
                </div>
                ...
                <div class="col-6 col-md-3 mt-6 mt-md-0 flex items-center justify-center mt-5 mt-md-0">
                    <div class="flex flex-col items-center text-center">
                        ...
                        <strong class="text-3xl text-dark-blue font-bold mt-5">{{ $finishedInstallmentsCount }}</strong>
                        <span class="font-16 text-slate-500 font-medium">{{ trans('update.finished_installments') }}</span>
                    </div>
                </div>
                ...
                <div class="col-6 col-md-3 mt-6 mt-md-0 flex items-center justify-center mt-5 mt-md-0">
                    <div class="flex flex-col items-center text-center">
                        ...
                        <strong class="text-3xl text-dark-blue font-bold mt-5">{{ $overdueInstallmentsCount }}</strong>
                        <span class="font-16 text-slate-500 font-medium">{{ trans('update.overdue_installments') }}</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <section class="mt-25">
        ...
        @if(!empty($orders) and count($orders))
            @foreach($orders as $order)
                ...
                @if(!empty($orderItem))
                    <div class="row mt-6">
                        <div class="col-12">
                            <div class="webinar-card webinar-list panel-installment-card flex">
                                <div class="image-box">
                                ...
                                <div class="webinar-card-body w-full flex flex-col">
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center">
                                            <h3 class="font-16 text-dark-blue font-bold">{{ $orderItem->title }}</h3>
                                            ...
                                            <div class="btn-group dropdown table-actions">
                                                ...
                                                <div class="dropdown-menu ">
                                                    ...
                                                    @if($order->status == "open")
                                                        <a href="/panel/financial/installments/{{ $order->id }}/pay_upcoming_part" target="_blank" class="webinar-actions block mt-10">{{ trans('update.pay_upcoming_part') }}</a>
                                                    @endif

                                                    @if(!in_array($order->status, ['refunded', 'canceled']))
                                                        <a href="/panel/financial/installments/{{ $order->id }}/details" target="_blank" class="webinar-actions block mt-10">{{ trans('update.view_details') }}</a>
                                                    @endif

                                                    @if($itemType == "course" and ($order->isCompleted() or $order->status == "open"))
                                                        <a href="{{ $orderItem->getLearningPageUrl() }}" target="_blank" class="webinar-actions block mt-10">{{ trans('update.learning_page') }}</a>
                                                    @endif

                                                    {{--@if($order->isCompleted() or $order->status == "open")
                                                        <a href="/panel/financial/installments/{{ $order->id }}/refund" class="webinar-actions block mt-10 delete-action">{{ trans('update.refund') }}</a>
                                                    @endif--}}

                                                    @if($order->status == "pending_verification" and getInstallmentsSettings("allow_cancel_verification"))
                                                        <a href="/panel/financial/installments/{{ $order->id }}/cancel" class="webinar-actions block mt-10 text-danger delete-action" data-title="{{ trans('public.deleteAlertHint') }}" data-confirm="{{ trans('update.yes_cancel') }}">{{ trans('public.cancel') }}</a>
                                                    @endif
                                                </div>
                                            </div>
                                        @endif
        ...
        @else
            @include('web.jiujitsu.includes.no-result',[
