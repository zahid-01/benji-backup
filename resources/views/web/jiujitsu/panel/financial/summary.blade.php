                                ...
                                @foreach($accountings as $accounting)
                                    <tr>
                                        <td class="text-left">
                                            <div class="flex flex-col">
                                                ...
                                                @if(!empty($accounting->gift_id) and !empty($accounting->gift))
                                                    <div class="text-slate-500 text-sm">{!! trans('update.a_gift_for_name_on_date',['name' => $accounting->gift->name, 'date' => dateTimeFormat($accounting->gift->date, 'j M Y H:i')]) !!}</div>
                                                @endif
                                        ...
                                        <td class="text-left align-middle">
                                            <span class="font-medium text-slate-500">{{ $accounting->description }}</span>
                                        </td>
                                        <td class="text-center align-middle">
                                            @switch($accounting->type)
                                                @case(\App\Models\Accounting::$addiction)
                                                    <span class="font-16 font-bold text-primary">+{{ handlePrice($accounting->amount, false) }}</span>
                                                    @break;
                                                @case(\App\Models\Accounting::$deduction)
                                                    <span class="font-16 font-bold text-danger">-{{ handlePrice($accounting->amount, false) }}</span>
                                                    @break;
                                            @endswitch
                                        </td>
