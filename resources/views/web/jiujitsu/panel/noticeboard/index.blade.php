...
@section('content')
                ...
                <div class="row">
                    <div class="col-4 flex items-center justify-center">
                        <div class="flex flex-col items-center text-center">
                            ...
                            <strong class="text-3xl font-bold mt-5">{{ $totalNoticeboards }}</strong>
                            <span class="font-16 text-dark-blue text-slate-500 font-medium">{{ trans('update.total_noticeboards') }}</span>
                        </div>
                    </div>
                    ...
                    <div class="col-4 flex items-center justify-center">
                        <div class="flex flex-col items-center text-center">
                            ...
                            <strong class="text-3xl font-bold mt-5">{{ $totalCourseNotices }}</strong>
                            <span class="font-16 text-dark-blue text-slate-500 font-medium">{{ trans('update.course_notices') }}</span>
                        </div>
                    </div>
                    ...
                    <div class="col-4 flex items-center justify-center">
                        <div class="flex flex-col items-center text-center">
                            ...
                            <strong class="text-3xl text-dark-blue font-bold mt-5">{{ $totalGeneralNotices }}</strong>
                            <span class="font-16 text-slate-500 font-medium">{{ trans('update.general_notices') }}</span>
                        </div>
                    </div>
                    ...
                    <div class="col-12 ">
                        <div class="table-responsive">
                            <table class="table custom-table text-center ">
                                <thead>
                                <tr>
                                    <th class="text-left text-slate-500">{{ trans('webinars.title') }}</th>
                                    <th class="text-center text-slate-500">{{ trans('site.message') }}</th>

                                    @if(!empty($isCourseNotice) and $isCourseNotice)
                                        <th class="text-center text-slate-500">{{ trans('update.color') }}</th>
                                    @else
                                        <th class="text-center text-slate-500">{{ trans('public.type') }}</th>
                                    @endif
                                ...
                                @foreach($noticeboards as $noticeboard)
                                    <tr class="noticeboard-item">
                                        <td class="text-left align-middle" width="25%">
                                            <span class="js-noticeboard-title block text-dark-blue font-medium">{{ $noticeboard->title }}</span>
                                            @if(!empty($noticeboard->webinar))
                                                <span class="block text-slate-500 text-sm">{{ $noticeboard->webinar->title }}</span>
                                            @endif
                                        </td>
                                        ...
                                        <td class="text-dark-blue font-medium align-middle">
                                            @if(!empty($isCourseNotice) and $isCourseNotice)
                                        ...
                                        <td class="js-noticeboard-time text-dark-blue font-medium align-middle">{{ dateTimeFormat($noticeboard->created_at,'j M Y | H:i') }}</td>
                                        <td class="text-right align-middle">
                                            <div class="btn-group dropdown table-actions">
                                                ...
                                                <div class="dropdown-menu">
                                                    <a href="/panel/{{ (!empty($isCourseNotice) and $isCourseNotice) ? 'course-noticeboard' : 'noticeboard' }}/{{ $noticeboard->id }}/edit" class="webinar-actions block mt-10 text-hover-primary">{{ trans('public.edit') }}</a>
                                                    <a href="/panel/{{ (!empty($isCourseNotice) and $isCourseNotice) ? 'course-noticeboard' : 'noticeboard' }}/{{ $noticeboard->id }}/delete" class="delete-action webinar-actions block mt-10 text-hover-primary">{{ trans('public.delete') }}</a>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
    ...
    <div class="hidden" id="noticeboardMessageModal">
        <div class="text-center">
            ...
            <span class="modal-time block text-sm text-slate-500 mt-15"></span>
            <p class="modal-message font-medium text-slate-500 mt-2"></p>
        </div>
    </div>
