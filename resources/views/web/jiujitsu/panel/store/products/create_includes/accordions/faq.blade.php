<li data-id="{{ !empty($faq) ? $faq->id :'' }}" class="accordion-row bg-white rounded-sm panel-shadow mt-4 py-15 py-lg-30 px-10 px-lg-20">
    <div class="flex items-center justify-between " role="tab" id="faq_{{ !empty($faq) ? $faq->id :'record' }}">
        <div class="flex items-center" href="#collapseFaq{{ !empty($faq) ? $faq->id :'record' }}" aria-controls="collapseFaq{{ !empty($faq) ? $faq->id :'record' }}" data-parent="#faqsAccordion" role="button" data-toggle="collapse" aria-expanded="true">
            <span class="chapter-icon chapter-content-icon mr-10">
                <i data-feather="help-circle" class=""></i>
            </span>

            <div class="font-bold text-dark-blue block">{{ !empty($faq) ? $faq->title : trans('webinars.add_new_faqs') }}</div>
        </div>
        ...
        <div class="flex items-center">
            ...
            @if(!empty($faq))
                <a href="/panel/store/products/faqs/{{ $faq->id }}/delete" class="delete-action btn btn-sm btn-ghost text-slate-500">
