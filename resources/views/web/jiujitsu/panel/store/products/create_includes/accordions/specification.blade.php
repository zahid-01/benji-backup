<li data-id="{{ !empty($selectedSpecification) ? $selectedSpecification->id :'' }}" class="accordion-row bg-white rounded-sm border border-gray300 mt-4 py-15 py-lg-30 px-10 px-lg-20">
    <div class="flex items-center justify-between " role="tab" id="specification_{{ !empty($selectedSpecification) ? $selectedSpecification->id :'record' }}">
        <div class="flex items-center" href="#collapseSpecification{{ !empty($selectedSpecification) ? $selectedSpecification->id :'record' }}" aria-controls="collapseSpecification{{ !empty($selectedSpecification) ? $selectedSpecification->id :'record' }}" data-parent="#specificationsAccordion" role="button" data-toggle="collapse" aria-expanded="true">
            <span class="chapter-icon chapter-content-icon mr-10">
                <i data-feather="file" class=""></i>
            </span>

            <div class="font-bold text-dark-blue block">{{ !empty($selectedSpecification) ? $selectedSpecification->specification->title : trans('update.add_new_specification') }}</div>
        </div>
        ...
        <div class="flex items-center">
            ...
            @if(!empty($selectedSpecification))
                <a href="/panel/store/products/specifications/{{ $selectedSpecification->id }}/delete" class="delete-action btn btn-sm btn-ghost text-slate-500">
                ...
                <div class="row">
                    <div class="col-12 col-lg-6">
                        ...
                        <div class="form-group mt-15">
                            <label class="input-label block">{{ trans('update.specification') }}</label>
                        ...
                        <div class="form-group js-multi-values-input  {{ (!empty($selectedSpecification) and $selectedSpecification->type == 'multi_value') ? '' : 'hidden' }}">
                            <label class="input-label block">{{ trans('update.parameters') }}</label>
                        ...
                        <div class="form-group js-summery-input {{ (!empty($selectedSpecification) and $selectedSpecification->type == 'textarea') ? '' : 'hidden' }}">
                            <label class="input-label block">{{ trans('update.product_summary') }}</label>
                        ...
                        <div class="form-group mt-4 js-allow-selection-input {{ (!empty($selectedSpecification) and $selectedSpecification->type == 'multi_value') ? '' : 'hidden' }}">
