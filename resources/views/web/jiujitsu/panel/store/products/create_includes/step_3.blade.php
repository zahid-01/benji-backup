...
<div class="row">
    ...
    @if($product->isVirtual())
        <div class="col-12 mb-40 mt-4">
            ...
            <div class="mt-15">
                <p class="text-sm text-slate-500">- {{ trans('update.product_files_hint_1') }}</p>
            </div>
                ...
                @if(!empty($product->files) and count($product->files))
                    <ul class="draggable-lists" data-order-path="/panel/store/products/files/order-items">
                        @foreach($product->files as $fileInfo)
                            @include('web.jiujitsu.panel.store.products.create_includes.accordions.file',['file' => $fileInfo])
                        @endforeach
                    </ul>
                @else
            ...
            <div id="newFileForm" class="hidden">
                @include('web.jiujitsu.panel.store.products.create_includes.accordions.file')
            </div>
