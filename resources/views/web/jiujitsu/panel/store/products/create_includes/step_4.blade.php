...
<div class="row">
    <div class="col-12 mt-4">
            ...
            @if(!empty($product->selectedSpecifications) and count($product->selectedSpecifications))
                <ul class="draggable-lists" data-order-path="/panel/store/products/specifications/order-items">
                    @foreach($product->selectedSpecifications as $selectedSpecificationRow)
                        @include('web.jiujitsu.panel.store.products.create_includes.accordions.specification',['selectedSpecification' => $selectedSpecificationRow])
                    @endforeach
                </ul>
            @else
        ...
        <div id="newSpecificationForm" class="hidden">
            @include('web.jiujitsu.panel.store.products.create_includes.accordions.specification')
        </div>
    </div>
    ...
    <div class="col-12 mt-14">
            ...
            @if(!empty($product->faqs) and count($product->faqs))
                <ul class="draggable-lists2" data-order-path="/panel/store/products/faqs/order-items">
                    @foreach($product->faqs as $faqRow)
                        @include('web.jiujitsu.panel.store.products.create_includes.accordions.faq',['faq' => $faqRow])
                    @endforeach
                </ul>
            @else
                @include(getTemplate() . '.includes.no-result',[
        ...
        <div id="newFaqForm" class="hidden">
            @include('web.jiujitsu.panel.store.products.create_includes.accordions.faq')
        </div>
    </div>
</div>
