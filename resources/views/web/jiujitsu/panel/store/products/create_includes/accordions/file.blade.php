<li data-id="{{ !empty($file) ? $file->id :'' }}" class="accordion-row bg-white rounded-sm border border-gray300 mt-4 py-15 py-lg-30 px-10 px-lg-20">
    <div class="flex items-center justify-between " role="tab" id="file_{{ !empty($file) ? $file->id :'record' }}">
        <div class="flex items-center" href="#collapseFile{{ !empty($file) ? $file->id :'record' }}" aria-controls="collapseFile{{ !empty($file) ? $file->id :'record' }}" data-parent="#filesAccordion" role="button" data-toggle="collapse" aria-expanded="true">
            <span class="chapter-icon chapter-content-icon mr-10">
                <i data-feather="file" class=""></i>
            </span>

            <div class="font-bold text-dark-blue block">{{ !empty($file) ? $file->title : trans('public.add_new_files') }}</div>
        </div>
        ...
        <div class="flex items-center">
            ...
            @if(!empty($file))
                <a href="/panel/store/products/files/{{ $file->id }}/delete" class="delete-action btn btn-sm btn-ghost text-slate-500">
            ...
            <div class="js-content-form file-form" data-action="/panel/store/products/files/{{ !empty($file) ? $file->id . '/update' : 'store' }}">
                ...
                <div class="row">
                    <div class="col-12 col-lg-6">
                        ...
                        <div class="js-online_viewer-input form-group mt-4 {{ (!empty($file) and $file->file_type == 'pdf') ? '' : 'hidden' }}">
