...
@section('content')
    <div class="">
        ...
        <form method="post" action="/panel/store/products/{{ !empty($product) ? $product->id .'/update' : 'store' }}" id="productForm" class="webinar-form">
            @include('web.jiujitsu.panel.store.products.create_includes.progress')
            ...
            @if($currentStep == 1)
                @include('web.jiujitsu.panel.store.products.create_includes.step_1')
            @elseif(!empty($product))
                @include('web.jiujitsu.panel.store.products.create_includes.step_'.$currentStep)
            @endif
        ...
        <div class="create-webinar-footer flex flex-col flex-md-row items-center justify-between mt-4 pt-15 border-top">
            ...
            <div class="mt-4 mt-md-0">
                ...
                @if(!empty($product) and $product->creator_id == $authUser->id)
                    <a href="/panel/store/products/{{ $product->id }}/delete?redirect_to=/panel/store/products" class="delete-action webinar-actions btn btn-sm bg-red-600 text-white mt-4 mt-md-0">{{ trans('public.delete') }}</a>
                @endif
            </div>
        </div>
    </div>
