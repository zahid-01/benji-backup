                    ...
                    <div class="col-12 ">
                        <div class="table-responsive">
                            <table class="table custom-table text-center ">
                                <thead>
                                <tr>
                                    <th class="text-left text-slate-500">{{ trans('update.product') }}</th>
                                    <th class="text-slate-500 text-center">{{ trans('panel.comment') }}</th>
                                    <th class="text-slate-500 text-center">{{ trans('public.status') }}</th>
                                    <th class="text-slate-500 text-center">{{ trans('public.date') }}</th>
                                    <th></th>
                                </tr>
                                </thead>
                                <tbody>
                                ...
                                @foreach($comments as $comment)
                                    <tr>
                                        <td class="text-left align-middle" width="35%">
                                            <a class="text-dark-blue font-medium" href="{{ $comment->product->getUrl() }}" target="_blank">{{ $comment->product->title }}</a>
                                        </td>
                                        ...
                                        <td class="align-middle">
                                            @if($comment->status == 'active')
                                                <span class="text-primary text-dark-blue font-medium">{{ trans('public.published') }}</span>
                                            @else
                                                <span class="text-warning text-dark-blue font-medium">{{ trans('public.pending') }}</span>
                                            @endif
                                        </td>

                                        <td class="text-dark-blue font-medium align-middle">{{ dateTimeFormat($comment->created_at,'j M Y | H:i') }}</td>
                                        <td class="align-middle text-right">
                                            ...
                                            <div class="btn-group dropdown table-actions">
                                                ...
                                                <div class="dropdown-menu">
                                                    <button type="button" data-comment-id="{{ $comment->id }}" class="js-edit-comment btn-ghost">{{ trans('public.edit') }}</button>
                                                    <a href="/panel/webinars/comments/{{ $comment->id }}/delete" class="delete-action btn-ghost block mt-10">{{ trans('public.delete') }}</a>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
