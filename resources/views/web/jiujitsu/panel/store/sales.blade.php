            ...
            <div class="row">
                ...
                <div class="col-6 col-md-3 flex items-center justify-center mt-5 mt-md-0">
                    <div class="flex flex-col items-center text-center">
                        ...
                        <strong class="text-3xl font-bold mt-5 text-dark-blue">{{ (!empty($totalSales) and $totalSales > 0) ? handlePrice($totalSales) : 0 }}</strong>
                        <span class="font-16 font-medium text-slate-500">{{ trans('financial.total_sales') }}</span>
                    </div>
                </div>
                                    ...
                                    <tr>
                                        <td class="text-left">
                                            <div class="user-inline-avatar flex items-center">
                                                ...
                                                <div class=" ml-2">
                                                    <span class="block">{{ !empty($order->buyer) ? $order->buyer->full_name : '' }}</span>
                                                    <span class="mt-5 text-sm text-slate-500 block">{{ !empty($order->buyer) ? $order->buyer->email : '' }}</span>
                                                </div>
                                            </div>
                                        </td>
                                            ...
                                            @if(!empty($order) and $order->status != \App\Models\ProductOrder::$canceled)
                                                <div class="btn-group dropdown table-actions">
                                                    ...
                                                    <div class="dropdown-menu font-weight-normal">
                                                        <a href="/panel/store/sales/{{ $order->sale_id }}/productOrder/{{ $order->id }}/invoice" class="webinar-actions block mt-10" target="_blank">{{ trans('public.invoice') }}</a>

                                                        @if($order->status == \App\Models\ProductOrder::$waitingDelivery)
                                                            <button type="button" data-sale-id="{{ $order->sale_id }}" data-product-order-id="{{ $order->id }}" class="js-enter-tracking-code webinar-actions btn-ghost block mt-10">{{ trans('update.enter_tracking_code') }}</button>
                                                        @endif
                                                    </div>
                                                </div>
                                            @endif
