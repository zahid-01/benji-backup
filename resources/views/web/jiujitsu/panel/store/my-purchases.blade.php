            ...
            <div class="row">
                <div class="col-6 col-md-3 flex items-center justify-center">
                    <div class="flex flex-col items-center text-center">
                        ...
                        <strong class="text-3xl font-bold mt-5 text-dark-blue">{{ $totalOrders }}</strong>
                        <span class="font-16 font-medium text-slate-500">{{ trans('update.total_orders') }}</span>
                    </div>
                </div>
                ...
                <div class="col-6 col-md-3 flex items-center justify-center">
                    <div class="flex flex-col items-center text-center">
                        ...
                        <strong class="text-3xl font-bold mt-5 text-dark-blue">{{ $pendingOrders }}</strong>
                        <span class="font-16 font-medium text-slate-500">{{ trans('update.pending_orders') }}</span>
                    </div>
                </div>
                ...
                <div class="col-6 col-md-3 flex items-center justify-center mt-5 mt-md-0">
                    <div class="flex flex-col items-center text-center">
                        ...
                        <strong class="text-3xl font-bold mt-5 text-dark-blue">{{ $canceledOrders }}</strong>
                        <span class="font-16 font-medium text-slate-500">{{ trans('update.canceled_orders') }}</span>
                    </div>
                </div>
                ...
                <div class="col-6 col-md-3 flex items-center justify-center mt-5 mt-md-0">
                    <div class="flex flex-col items-center text-center">
                        ...
                        <strong class="text-3xl font-bold mt-5 text-dark-blue">{{ !empty($totalPurchase) ? handlePrice($totalPurchase) : 0 }}</strong>
                        <span class="font-16 font-medium text-slate-500">{{ trans('update.total_purchase') }}</span>
                    </div>
                </div>
                                    ...
                                    <tr>
                                        <td class="text-left">
                                            <div class="user-inline-avatar flex items-center">
                                                ...
                                                <div class=" ml-2">
                                                    <span class="block">{{ !empty($order->seller) ? $order->seller->full_name : '' }}</span>
                                                    <span class="mt-5 text-sm text-slate-500 block">{{ !empty($order->seller) ? $order->seller->email : '' }}</span>
                                                </div>
                                            </div>
                                        </td>

                                        <td class=" text-left">
                                            <span class="block font-medium text-dark-blue font-16">{{ $order->id }}</span>
                                            <span class="block text-sm text-slate-500">{{ $order->quantity }} {{ trans('update.product') }}</span>
                                        </td>
                                            ...
                                            @if(!empty($order) and $order->status != \App\Models\ProductOrder::$canceled)
                                                <div class="btn-group dropdown table-actions">
                                                    ...
                                                    <div class="dropdown-menu font-weight-normal">
                                                        <a href="/panel/store/purchases/{{ $order->sale_id }}/productOrder/{{ $order->id }}/invoice" target="_blank" class="webinar-actions block mt-10">{{ trans('public.invoice') }}</a>

                                                        @if(!empty($order->product) and $order->status == \App\Models\ProductOrder::$success)
                                                            <a href="{{ $order->product->getUrl() }}" class="webinar-actions block mt-10" target="_blank">{{ trans('public.feedback') }}</a>
                                                        @endif

                                                        @if($order->status == \App\Models\ProductOrder::$shipped)
                                                            <button type="button" data-sale-id="{{ $order->sale_id }}" data-product-order-id="{{ $order->id }}" class="js-view-tracking-code webinar-actions btn-ghost block mt-10">{{ trans('update.view_tracking_code') }}</button>

                                                            <button type="button" data-sale-id="{{ $order->sale_id }}" data-product-order-id="{{ $order->id }}" class="js-got-the-parcel webinar-actions btn-ghost block mt-10">{{ trans('update.i_got_the_parcel') }}</button>
                                                        @endif
                                                    </div>
                                                </div>
                                            @endif
