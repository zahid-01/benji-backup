            ...
            <div class="row">
                <div class="col-6 col-md-3 flex items-center justify-center">
                    <div class="flex flex-col items-center text-center">
                        ...
                        <strong class="text-3xl text-dark-blue font-bold mt-5">{{ $quizzesResultsCount }}</strong>
                        <span class="font-16 text-slate-500 font-medium">{{ trans('quiz.quizzes') }}</span>
                    </div>
                </div>
                ...
                <div class="col-6 col-md-3 mt-6 mt-md-0 flex items-center justify-center mt-5 mt-md-0">
                    <div class="flex flex-col items-center text-center">
                        ...
                        <span class="font-16 text-slate-500 font-medium">{{ trans('quiz.open_results') }}</span>
                    </div>
                </div>
                                    ...
                                    <tr>
                                        <td class="text-left">
                                            <div class="user-inline-avatar flex items-center">
                                                ...
                                                <div class=" ml-2">
                                                    <span class="block">{{ $result->quiz->creator->full_name }}</span>
                                                    <span class="mt-5 text-sm text-slate-500 block">{{ $result->quiz->creator->email }}</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="text-left">
                                        ...
                                        <span class="block text-{{ ($result->status == 'passed') ? 'primary' : ($result->status == 'waiting' ? 'warning' : 'danger') }}">
                                            {{ trans('quiz.'.$result->status) }}
                                        </span>

                                            @if($result->status =='failed' and $result->can_try)
                                                <span class="block text-sm text-slate-500">{{ trans('quiz.quiz_chance_remained',['count' => $result->count_can_try]) }}</span>
                                            @endif
                                        </td>
                                            ...
                                            <div class="btn-group dropdown table-actions table-actions-lg table-actions-lg">
                                                ...
                                                <div class="dropdown-menu">
                                                    @if((!$result->can_try and $result->status != 'waiting') or ($result->status == 'passed'))
                                                        <a href="/panel/quizzes/{{ $result->id }}/result" class="webinar-actions block mt-10">{{ trans('public.view_answers') }}</a>
                                                    @endif

                                                    @if($result->status != 'passed')
                                                        @if($result->can_try)
                                                            <a href="/panel/quizzes/{{ $result->quiz->id }}/start" class="webinar-actions block mt-10">{{ trans('public.try_again') }}</a>
                                                        @endif
                                                    @endif

                                                    <a href="{{ $result->quiz->webinar->getUrl() }}" class="webinar-actions block mt-10">{{ trans('webinars.webinar_page') }}</a>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
