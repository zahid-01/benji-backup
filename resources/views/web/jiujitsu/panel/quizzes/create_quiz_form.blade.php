<div class="">
    <div data-action="{{ !empty($quiz) ? ('/panel/quizzes/'. $quiz->id .'/update') : ('/panel/quizzes/store') }}" class="js-content-form quiz-form webinar-form">
            ...
            <div class="row">
                <div class="col-12 col-md-4">
                    ...
                    <div class="form-group">
                        ...
                        <p class="text-sm text-slate-500 mt-5">{{ trans('update.quiz_expiry_days_hint') }}</p>
                    </div>
                    ...
                    @if(!empty($quiz))
                        ...
                        <div class="form-group js-display-limited-questions-count-field {{ ($quiz->display_limited_questions) ? '' : 'hidden' }}">
        ...
        @if(!empty($quiz))
            <section class="mt-6">
                <div class="block d-mflex justify-between items-center pb-20">
                    <h2 class="section-title after-line">{{ trans('public.questions') }}</h2>

                    <div class="flex items-center mt-4 mt-md-0">
                ...
                @if($quizQuestions)
                    <ul class="draggable-questions-lists draggable-questions-lists-{{ $quiz->id }}" data-drag-class="draggable-questions-lists-{{ $quiz->id }}" data-order-table="quizzes_questions" data-quiz="{{ $quiz->id }}">
                        @foreach($quizQuestions as $question)
                            <li data-id="{{ $question->id }}" class="quiz-question-card flex items-center mt-4">
                                <div class="grow">
                                    <h4 class="question-title">{{ $question->title }}</h4>
                                    <div class="text-sm mt-5 question-infos">
                                ...
                                <div class="btn-group dropdown table-actions">
                                    ...
                                    <div class="dropdown-menu">
                                        <button type="button" data-question-id="{{ $question->id }}" class="edit_question btn btn-sm btn-ghost block">{{ trans('public.edit') }}</button>
                                        <a href="/panel/quizzes-questions/{{ $question->id }}/delete" class="delete-action btn btn-sm btn-ghost block">{{ trans('public.delete') }}</a>
                                    </div>
                                </div>
                            </li>
                        @endforeach
