<div class="@if(!empty($quiz)) descriptiveQuestionModal{{ $quiz->id }} @endif {{ empty($question_edit) ? 'hidden' : ''}}">
    <div class="custom-modal-body">
        ...
        <div class="quiz-questions-form" data-action="/panel/quizzes-questions/{{ empty($question_edit) ? 'store' : $question_edit->id.'/update' }}">
            ...
            <p class="text-sm text-slate-500 col-12 pb-10">{{ trans('update.quiz_question_image_validation_by_video') }}</p>
            <div class="row">
                <div class="col-12">
                    <div class="form-group">
            ...
            <div class="flex items-center justify-end mt-25">
                <button type="button" class="save-question btn btn-sm btn-primary">{{ trans('public.save') }}</button>
                <button type="button" class="close-swl btn btn-sm bg-red-600 text-white ml-10">{{ trans('public.close') }}</button>
            </div>
        </div>
    </div>
</div>
