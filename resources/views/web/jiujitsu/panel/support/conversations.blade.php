            ...
            <form action="/panel/support" method="get">
                <div class="row">
                    ...
                    @if(!$authUser->isUser())
                        ...
                        <div id="studentSelectInput" class="col-12 col-md-4 col-lg-2 @if(request()->get('role') != 'student' and (empty(request()->get('student'))  or request()->get('student') == 'all')) hidden @endif">
                            <div class="form-group">
                    ...
                    <div id="teacherSelectInput" class="col-12 col-md-4 col-lg-2 @if(!$authUser->isUser() and request()->get('role') != 'teacher' and (empty(request()->get('teacher')) or request()->get('teacher') == 'all')) hidden @endif">
                        <div class="form-group">
                    ...
                    <div id="conversationsList" class="col-12 col-lg-6 conversations-list">
                        <div class="table-responsive">
                            <table class="table table-md">
                                <tr>
                                    <th class="text-left text-slate-500 text-sm font-medium">{{ trans('navbar.contact') }}</th>
                                    <th class="text-left text-slate-500 text-sm font-medium">{{ trans('public.title') }}</th>
                                    <th class="text-center text-slate-500 text-sm font-medium">{{ trans('public.status') }}</th>
                                </tr>
                                <tbody>
                                ...
                                @foreach($supports as $support)
                                    <tr class="@if(!empty($selectSupport) and $selectSupport->id == $support->id) selected-row @endif">
                                        <td class="text-left">
                                            <a href="/panel/support/{{ $support->id }}/conversations" class="">
                                                <div class="user-inline-avatar flex items-center">
                                                    ...
                                                    <div class="ml-10">
                                                        <span class="block text-sm text-dark-blue font-medium">{{ (!empty($support->webinar) and $support->webinar->teacher_id != $authUser->id) ? $support->webinar->teacher->full_name : $support->user->full_name }}</span>
                                                        <span class="mt-1 text-sm text-slate-500 block">
                                        ...
                                        <td class="text-left">
                                            @if($authUser->isUser())
                                                <a href="/panel/support/{{ $support->id }}/conversations" class="">
                                                    <span class="font-medium text-sm text-dark-blue block">{{ $support->title }}</span>
                                                    <span class="mt-1 text-sm text-slate-500 block">{{ truncate((!empty($support->webinar)) ? $support->webinar->title : '', 20) }} | {{ (!empty($support->conversations) and count($support->conversations)) ? dateTimeFormat($support->conversations->first()->created_at,'j M Y | H:i') : dateTimeFormat($support->created_at,'j M Y | H:i') }}</span>
                                                </a>
                                            @else
                                                <a href="/panel/support/{{ $support->id }}/conversations" class="">
                                                    <span class="font-medium text-sm text-dark-blue block">{{ $support->title }}</span>
                                                    <span class="mt-1 text-sm text-slate-500 block">{{ (!empty($support->conversations) and count($support->conversations)) ? dateTimeFormat($support->conversations->first()->created_at,'j M Y | H:i') : dateTimeFormat($support->created_at,'j M Y | H:i') }}</span>
                                                </a>
                                            @endif
                                        </td>
                                        ...
                                        <td class="text-center align-middle">
                                            @if($support->status == 'close')
                                                <span class="text-danger font-medium text-sm">{{  trans('panel.closed') }}</span>
                                            @elseif($support->status == 'supporter_replied')
                                                <span class="text-primary font-medium text-sm">{{  trans('panel.replied') }}</span>
                                            @else
                                                <span class="text-warning font-medium text-sm">{{  trans('public.waiting') }}</span>
                                            @endif
                                        </td>
                        ...
                        <div class="col-12 col-lg-6 border-left border-gray300">
                            <div class="conversation-box p-15 flex items-center justify-between">
                                <div>
                                    ...
                                    <span class="text-sm mt-1 text-slate-500 block">{{ trans('public.created') }}: {{ dateTimeFormat($support->created_at,'j M Y | H:i') }}</span>
                                ...
                                @if(!empty($selectSupport->conversations) and !$selectSupport->conversations->isEmpty())
                                    ...
                                    @foreach($selectSupport->conversations as $conversations)
                                        <div class="rounded-sm mt-15 panel-shadow border p-15">
                                            ...
                                            <p class="white-space-pre-wrap text-slate-500 text-sm mt-15 font-medium">{{ $conversations->message }}</p>
                                        </div>
                                    @endforeach
