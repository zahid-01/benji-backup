...
@section('content')
    ...
    @if(!empty($cashbackRules) and count($cashbackRules))
        <div class="container relative mt-6">
            @include('web.jiujitsu.includes.cashback_alert',['itemPrice' => $product->price])
        </div>
    @endif

    <div class="container product-show-special-offer relative mt-6">
        @if(!empty($activeSpecialOffer))
            @include('web.jiujitsu.course.special_offer')
        @endif
    </div>

    <div class="container {{ !empty($activeSpecialOffer) ? 'mt-50' : 'mt-6' }}">
        <div class="row">
            <div class="col-12 col-lg-6">
                <div class="lazyImage product-show-image-card relative">
                    ...
                    @if(!empty($product->video_demo))
                        <button id="productDemoVideoBtn"
                                ...
                                class="product-video-demo-icon cursor-pointer btn-ghost flex items-center justify-center">
                ...
                <div class="product-show-thumbnail-card flex items-center mt-4">
                    <div class="thumbnail-card is-first-thumbnail-card cursor-pointer relative">
                        <img src="{{ $product->thumbnail }}" alt="{{ $product->title }}" class="img-cover rounded-sm">

                        @if(!empty($product->video_demo))
                            <span class="product-video-demo-thumb-icon flex items-center justify-center">
                                <img src="/assets/default/img/icons/play-bold.svg" alt="play icon" class=""/>
                            </span>
                        @endif
                    </div>
            ...
            <div class="col-12 col-lg-6 mt-4 mt-lg-0">
                <form action="/cart/store" method="post" id="productAddToCartForm">
                    ...
                    <div class="product-show-info-card bg-info p-15 p-md-25 rounded-lg">
                        ...
                        <span class="block font-16 mt-10">{{ trans('public.in') }} <a href="{{ $product->category->getUrl() }}" target="_blank" class="font-medium text-decoration-underline">{{ $product->category->title }}</a></span>

                        <div class="flex flex-col flex-md-row align-items-md-center justify-content-md-between mt-4">
                            <div class="flex items-center">
                                @include('web.jiujitsu.includes.webinar.rate',['rate' => $product->getRate(),'className' => 'mt-0'])
                                <span class="ml-10 text-sm">({{ $product->reviews->pluck('creator_id')->count() }} {{ trans('product.reviews') }})</span>
                            </div>

                            <div class="flex items-center mt-15 mt-md-0">
                        ...
                        @if(!empty($selectableSpecifications) and count($selectableSpecifications))
                            @foreach($selectableSpecifications as $selectableSpecification)
                                <div class="product-show-selectable-specification mt-10">
                                    <span class="text-sm font-bold text-dark">{{ $selectableSpecification->specification->title }}</span>
                                        ...
                                        @foreach($selectableSpecification->selectedMultiValues as $specificationValue)
                                            @if(!empty($specificationValue->multiValue))
                                                <div class="selectable-specification-item mr-5 mt-5">
                                                    ...
                                                    <label class="text-sm cursor-pointer px-10 py-5" for="{{ $specificationValue->multiValue->createName() }}">{{ $specificationValue->multiValue->title }}</label>
                                                </div>
                                            @endif
                                        @endforeach
                                    </div>
                        ...
                        <div class="product-show-price-box mt-15">
                            ...
                            @if($product->isPhysical())
                                @if(!empty($product->delivery_fee) and $product->delivery_fee > 0)
                                    <span class="shipping-price block mt-5">+ {{ handlePrice($product->delivery_fee) }} {{ trans('update.shipping') }}</span>
                                @else
                                    <span class="text-warning block text-sm font-medium mt-5">{{ trans('update.free_shipping') }}</span>
                                @endif
                            @endif
                        </div>

                        <div class="product-show-cart-actions flex items-center flex-wrap ">
                            <div class="cart-quantity flex items-center mt-4 mr-15">
                                <input type="hidden" id="productAvailabilityCount" value="{{ $product->getAvailability() }}">
                                <button type="button" class="minus flex items-center justify-center" {{ ($product->getAvailability() < 1) ? 'disabled' : '' }}>
                                ...
                                <button type="button" class="plus flex items-center justify-center" {{ ($product->getAvailability() < 1) ? 'disabled' : '' }}>
                            ...
                            <div class="flex flex-col flex-md-row flex-md-wrap align-items-md-center w-full">
                                <button type="submit" class="btn mt-4 {{ ($productAvailability > 0) ? 'btn-primary' : 'btn-dark' }}" {{ ($productAvailability < 1) ? 'disabled' : '' }}>
                                ...
                                @if($productAvailability > 0 and !empty($product->point) and $product->point > 0)
                                    ...
                                    <a href="{{ !(auth()->check()) ? '/login' : '#!' }}" class="{{ (auth()->check()) ? 'js-buy-with-point' : '' }} js-buy-with-point-show-btn btn btn-outline-warning mt-4 ml-0 ml-md-10" rel="nofollow">
                                ...
                                @if($productAvailability > 0 and !empty(getFeaturesSettings('direct_products_payment_button_status')))
                                    <button type="button" class="btn btn-outline-danger mt-4 ml-0 ml-md-10 js-product-direct-payment">
                                        {{ trans('update.buy_now') }}
                                    </button>
                                @endif

                                @if($productAvailability > 0 and $hasInstallments)
                                    <a href="/products/{{ $product->slug }}/installments" class="js-installments-btn btn btn-outline-primary mt-4 ml-0 ml-md-10">
                        ...
                        <div class="flex flex-col flex-md-row align-items-md-center w-full mt-8">
                            @if($product->isPhysical() and !empty($product->delivery_estimated_time))
                                <div class="product-show-info-footer-items flex items-center mb-10 mb-md-0 mr-0 mr-md-10">
                                    <div class="icon-box">
                                        <i data-feather="package" class="" width="20" height="20"></i>
                                    </div>
                                    <div class="ml-2">
                                        <span class="block text-sm font-bold text-dark">{{ trans('update.physical_product') }}</span>
                                        <span class="block text-sm text-slate-500">{{ trans('update.delivery_estimated_time_days_alert',['days' => $product->delivery_estimated_time]) }}</span>
                                    </div>
                                </div>
                            @elseif($product->isVirtual())
                                <div class="product-show-info-footer-items flex items-center mb-10 mb-md-0 mr-0 mr-md-10">
                                    ...
                                    <div class="ml-2">
                                        <span class="block text-sm font-bold text-dark">{{ trans('update.virtual_product') }}</span>
                                        <span class="block text-sm text-slate-500">{{ trans('update.download_all_files_after_payment') }}</span>
                                    </div>
                                </div>
                            @endif

                            <div class="js-share-product product-show-info-footer-items flex items-center cursor-pointer">
                                <div class="icon-box">
                                    <i data-feather="share-2" class="" width="20" height="20"></i>
                                </div>
                                <div class="ml-2">
                                    <span class="block text-sm font-bold text-dark">{{ trans('public.share') }}</span>
                                    <span class="block text-sm text-slate-500">{{ trans('update.product_share_text') }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        @if($product->isVirtual() and $productAvailability > 0 and !empty(getGiftsGeneralSettings('status')) and !empty(getGiftsGeneralSettings('allow_sending_gift_for_products')))
                            <a href="/gift/product/{{ $product->slug }}" class="flex items-center mt-15 rounded-lg border p-15">
                                <div class="size-40 flex-center rounded-full bg-gray200">
                                    <i data-feather="gift" class="text-slate-500" width="20" height="20"></i>
                                </div>
                                <div class="ml-2">
                                    <h4 class="text-sm font-bold text-slate-500">{{ trans('update.gift_this_product') }}</h4>
                                    <p class="text-sm text-slate-500">{{ trans('update.gift_this_product_hint') }}</p>
                                </div>
                            </a>
                        @endif
        ...
        <div class="mt-6">
            <ul class="product-show__nav-tabs nav nav-tabs p-15 flex items-center" id="tabs-tab" role="tablist">
                <li class="nav-item mr-20 mr-lg-30">
                    <a class="relative text-sm {{ (empty(request()->get('tab')) or request()->get('tab') == 'description') ? 'active' : '' }}" id="description-tab"
                       data-toggle="tab" href="#description" role="tab" aria-controls="description"
                       aria-selected="true">{{ trans('public.description') }}</a>
                </li>
                <li class="nav-item mr-20 mr-lg-30">
                    <a class="relative text-sm {{ (request()->get('tab') == 'seller') ? 'active' : '' }}" id="seller-tab" data-toggle="tab"
                       href="#seller" role="tab" aria-controls="seller"
                       aria-selected="false">{{ trans('update.seller') }}</a>
                </li>
                <li class="nav-item mr-20 mr-lg-30">
                    <a class="relative text-sm {{ (request()->get('tab') == 'specifications') ? 'active' : '' }}" id="specifications-tab" data-toggle="tab"
                ...
                @if(!empty($product->files) and count($product->files) and $product->checkUserHasBought())
                    <li class="nav-item mr-20 mr-lg-30">
                        <a class="relative text-sm {{ (request()->get('tab') == 'files') ? 'active' : '' }}" id="files-tab" data-toggle="tab"
                ...
                <li class="nav-item mr-20 mr-lg-30">
                    <a class="relative text-sm {{ (request()->get('tab') == 'reviews') ? 'active' : '' }}" id="reviews-tab" data-toggle="tab"
    ...
    @include('web.jiujitsu.products.includes.share_modal')
    @include('web.jiujitsu.user.send_message_modal',['user' => $seller])
    @include('web.jiujitsu.products.includes.buy_with_point_modal')
@endsection
