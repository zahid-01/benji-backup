@if(!empty($authUser) and ($authUser->isOrganization() or $authUser->isTeacher()))
    <a href="/panel/store/products/new" class="mt-4 btn btn-primary btn-flex items-center w-full">
...
<div class="mt-4 p-20 rounded-sm shadow-lg border border-gray300 filters-container">
    <div class="">
        <h3 class="category-filter-title font-20 font-bold text-dark-blue">{{ trans('update.options') }}</h3>
            ...
            <div class="pt-10">
                @foreach($productCategories as $productCategory)
                    @if(!empty($productCategory->subCategories) and count($productCategory->subCategories))
                        ...
                        <span class="block text-sm font-bold  mt-4">{{ $productCategory->title }}</span>

                        <div class="pl-10">
                            @foreach($productCategory->subCategories as $subCategory)
                                <a href="{{ $subCategory->getUrl() }}" class="flex items-center text-sm font-weight-normal mt-4 {{ (!empty($selectedCategory) and $selectedCategory->id == $subCategory->id) ? 'text-primary' : '' }}">
                    ...
                    @else
                        <a href="{{ $productCategory->getUrl() }}" class="flex items-center text-sm font-bold mt-4 {{ (!empty($selectedCategory) and $selectedCategory->id == $productCategory->id) ? 'text-primary' : '' }}">
