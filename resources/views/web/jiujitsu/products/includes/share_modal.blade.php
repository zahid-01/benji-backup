<div class="hidden" id="productShareModal">
    ...
    <div class="text-center">
        ...
        <p class="mt-4 text-sm">{{ trans('update.share_this_product_with_others') }}</p>

        <div class="relative flex items-center justify-between p-15 mt-15 border border-gray250 rounded-sm mt-5">
            <div class="js-product-share-link font-bold px-16 text-ellipsis text-sm">{{ $product->getUrl() }}</div>

            <button type="button" class="js-product-share-link-copy btn btn-primary btn-sm text-sm font-medium flex-none" data-toggle="tooltip" data-placement="top" title="{{ trans('public.copy') }}">{{ trans('public.copy') }}</button>
        </div>
