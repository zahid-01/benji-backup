...
@section('content')
    <div class="container">
        <section class="topics-title-section mt-6 mt-md-50 px-20 px-md-30 py-25 py-md-35 rounded-lg">
            <h1 class="text-3xl font-bold text-white">{{ !empty($topic) ? trans('update.edit_topic') : trans('update.new_topic') }}</h1>
            <p class="text-sm text-white">{{ trans('update.new_topic_hint') }}</p>
            ...
            <div class="mt-10">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb p-0 m-0">
                        ...
                        <li class="breadcrumb-item text-sm text-white font-bold" aria-current="page">{{ !empty($topic) ? trans('update.edit_topic') : trans('update.new_topic') }}</li>
                    </ol>
                </nav>
            </div>
        </section>
        ...
        <form action="{{ !empty($topic) ? $topic->getEditUrl() : '/forums/create-topic' }}" method="post">
            ...
            <div class="rounded-lg px-15 py-20 border bg-white mt-4">
                <div class="row">
                            ...
                            @if(!empty($topic) and !empty($topic->attachments) and count($topic->attachments))
                                @foreach($topic->attachments as $topicAttachment)
                                    <div class="input-group product-images-input-group mt-10">
                                        ...
                                        <button type="button" class="btn btn-sm bg-red-600 text-white remove-btn">
            ...
            <div class="mt-15 p-10 bg-info-light rounded-lg flex items-center justify-between">
                <div class="py-5">
                    <div class="text-sm font-bold text-slate-500">{{ trans('update.terms_and_rules_confirmation') }}</div>
                    <p class="block text-sm text-slate-500 mt-5">{{ trans('update.terms_and_rules_confirmation_hint') }}</p>
                </div>
