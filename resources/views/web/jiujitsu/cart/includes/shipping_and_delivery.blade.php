    ...
    <div class="rounded-sm shadow mt-4 py-25 px-20">
        <div class="row">
            ...
            <div class="col-12 {{ !empty(getStoreSettings('show_address_selection_in_cart')) ? 'col-lg-6' : '' }}">
                <div class="form-group">
                    <label class="input-label font-medium">{{ trans('update.address') }}</label>
                ...
                <div class="form-group">
                    <label class="input-label font-medium">{{ trans('update.message_to_seller') }}</label>
...
@if(!empty($deliveryEstimateTime))
    <div class="flex items-center mt-6 rounded-lg border px-10 py-5">
        <div class="appointment-timezone-icon">
            <img src="/assets/default/img/icons/timezone.svg" alt="appointment timezone">
        </div>
        <div class="ml-15">
            <div class="font-16 font-bold text-dark-blue">{{ trans('update.cart_order_estimated_delivery_time') }}</div>
            <p class="text-sm font-medium text-slate-500">{{ trans('update.cart_order_estimated_delivery_time_hint',['days' => $deliveryEstimateTime]) }}</p>
        </div>
    </div>
@endif
