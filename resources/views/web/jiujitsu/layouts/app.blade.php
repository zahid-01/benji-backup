...
<div id="app" class="{{ (!empty($floatingBar) and $floatingBar->position == 'top' and $floatingBar->fixed) ? 'has-fixed-top-floating-bar px-3' : 'px-3' }}">
    @if(!empty($floatingBar) and $floatingBar->position == 'top')
        {{-- @include('web.jiujitsu.includes.floating_bar') --}}
    @endif

    @if(!isset($appHeader))
        {{-- @include('web.jiujitsu.includes.top_nav') --}}
    ...
    <div class="flex gap-4 xl:gap-8">
        @include('web.jiujitsu.includes.side_nav')
        <div class="w-full">
            @yield('content')
        </div>
    </div>
