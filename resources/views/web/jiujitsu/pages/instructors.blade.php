...
@section('content')
            ...
            <div class="row h-100 items-center justify-center text-center">
                <div class="col-12 col-md-9 col-lg-7">
                    <div class="top-search-categories-form">
                        <h1 class="text-white text-3xl mb-15">{{ $title }}</h1>
                        ...
                        <div class="search-input bg-white p-10 grow">
                            <form action="/{{ $page }}" method="get">
                                <div class="form-group flex items-center m-0">
                                    <input type="text" name="search" class="form-control border-slate-300 input" value="{{ request()->get('search') }}" placeholder="{{ trans('public.search') }} {{ $title }}"/>
    ...
    <div class="container max-w-5xl">
        ...
        <form id="filtersForm" action="/{{ $page }}" method="get">
            ...
            <div id="topFilters" class="mt-25 shadow-lg border rounded-md border-slate-300">
                <div class="row">
                    <div class="col-lg-6 flex gap-4">
                        ...
                        <div class="gap-12 text-right"></div>
            ...
            <div class="mt-6 px-6 py-15 border rounded-md shadow-lg border-slate-300">
                <h3 class="category-filter-title font-20 font-bold mt-3">{{ trans('categories.categories') }}</h3>
                    ...
                    @foreach($categories as $category)
                        @if(!empty($category->subCategories) and count($category->subCategories))
                            @foreach($category->subCategories as $subCategory)
                                <div class="checkbox-button bordered-200 mb-2 mr-3">
                            ...
                            @endforeach
                        @else
                            <div class="checkbox-button bordered-200 mr-3">
        ...
        <section>
            <div id="instructorsList" class="row mt-4">
            ...
            <div class="text-center">
                <button type="button" id="loadMoreInstructors" data-page="{{ ($page == 'instructors') ? \App\Models\Role::$teacher : \App\Models\Role::$organization }}" class="btn btn-border-white mt-50 {{ ($instructors->lastPage() <= $instructors->currentPage()) ? ' hidden' : '' }}">{{ trans('site.load_more_instructors') }}</button>
            </div>
        </section>
        ...
        @if(!empty($bestRateInstructors) and !$bestRateInstructors->isEmpty() and (empty(request()->get('sort')) or !in_array(request()->get('sort'),['top_rate','top_sale'])))
            <section class="mt-6 pt-30">
                <div class="flex justify-between">
                    <div>
                        <h2 class="text-xl font-semibold text-dark-blue">{{ trans('site.best_rated_instructors') }}</h2>
                        <span class="text-sm text-slate-500">{{ trans('site.best_rated_instructors_subtitle') }}</span>
                    </div>
                ...
                <div class="relative mt-4 flex">
                    <div id="bestRateInstructorsSwiper" class="swiper-container px-3">
                        <div class="swiper-wrapper pb-2">

                            @foreach($bestRateInstructors as $bestRateInstructor)
                                <div class="swiper-slide">
                                    @include('web.jiujitsu.pages.instructor_card',['instructor' => $bestRateInstructor])
                                </div>
                            @endforeach
        ...
        @if(!empty($bestSalesInstructors) and !$bestSalesInstructors->isEmpty() and (empty(request()->get('sort')) or !in_array(request()->get('sort'),['top_rate','top_sale'])))
            <section class="mt-50 pt-50 mb-10">
                <div class="flex justify-between">
                    <div>
                        <h2 class="text-xl font-semibold text-dark-blue">{{ trans('site.top_sellers') }}</h2>
                        <span class="text-sm text-slate-500">{{ trans('site.top_sellers_subtitle') }}</span>
                    </div>
                    ...
                    <div id="topSaleInstructorsSwiper" class="swiper-container px-12">
                        <div class="swiper-wrapper pb-20">
                            ...
                            @foreach($bestSalesInstructors as $bestSalesInstructor)
                                <div class="swiper-slide">
                                    @include('web.jiujitsu.pages.instructor_card',['instructor' => $bestSalesInstructor])
                                </div>
                            @endforeach
